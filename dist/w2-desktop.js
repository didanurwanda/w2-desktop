/* W2-desktop 1.0.1 - http://w2desktop.didanurwanda.com  */
...
var w2utils = (function ($) {
    ...
    function lock (box, msg, spinner) {
        ...
        w2utils.unlock(box);
        $(box).prepend(
            '<div class="wd-lock"></div>'+
            '<div class="wd-lock-msg"></div>'
        );
        var $lock = $(box).find('.wd-lock');
        var mess = $(box).find('.wd-lock-msg');
        if (!options.msg) mess.css({ 'background-color': 'transparent', 'border': '0px' });
        if (options.spinner === true) options.msg = '<div class="wd-spinner" '+ (!options.msg ? 'style="width: 35px; height: 35px"' : '') +'></div>' + options.msg;
    ...
    function unlock (box, speed) {
        if (isInt(speed)) {
            $(box).find('.wd-lock').fadeOut(speed);
            setTimeout(function () {
                $(box).find('.wd-lock').remove();
                $(box).find('.wd-lock-msg').remove();
            }, speed);
        } else {
            $(box).find('.wd-lock').remove();
            $(box).find('.wd-lock-msg').remove();
...
(function ($) {
    ...
    $.fn.w2render = function (name) {
        if ($(this).length > 0) {
            if (typeof name === 'string' && wd[name]) wd[name].render($(this)[0]);
    ...
    $.fn.w2destroy = function (name) {
        ...
        if (typeof name === 'string' && wd[name]) wd[name].destroy();
        ...
        if (str.length == 0 || !str[0]) { // remove marker
            ...
            return $(this).each(function (index, el) {
                ...
                function replaceValue(matched) { // mark new
                    return '<span class="wd-marker">' + matched + '</span>';
                }
            });
        }

        function clearMarkedText(index, el) {
            while (el.innerHTML.indexOf('<span class="wd-marker">') != -1) {
                el.innerHTML = el.innerHTML.replace(/\<span class=\"wd\-marker\"\>(.*)\<\/span\>/ig, '$1'); // unmark
            }
        }
    };
    ...
    $.fn.w2tag = function (text, options) {
        ...
        return $(this).each(function (index, el) {
            ...
            if (text === '' || text == null) {
                ...
                // insert
                $('body').append(
                    '<div onclick="event.stopPropagation()" id="wd-tag-'+ origID +'" class="wd-tag '+ ($(el).parents('.wd-popup').length > 0 ? 'wd-tag-popup' : '') + '">'+
                    '   <div style="margin: -2px 0px 0px -2px; white-space: nowrap;">'+
                    '      <div class="wd-tag-body '+ options.className +'" style="'+ (options.style || '') +'">'+ text +'</div>'+
                    '   </div>' +
                    '</div>');
                $tags = $('#wd-tag-'+tagID);
            ...
            setTimeout(function () {
                ...
                $tags.css({
                    ...
                    .find('.wd-tag-body').addClass(pos['posClass']);
            ...
            function checkIfMoved(skipTransition) {
                ...
                if ($(el).length === 0 || (offset.left === 0 && offset.top === 0) || $tags.find('.wd-tag-body').length == 0) {
                    clearInterval($tags.data('timer'));
                    hideTag();
                    return;
                }
                ...
                var tagBody  = $tags.find('.wd-tag-body');
    ...
    $.fn.w2overlay = function (html, options) {
        ...
        if (this.length == 0 || options.html === '' || options.html == null) {
            if ($('#wd-overlay'+ name).length > 0) {
                tmp_hide = $('#wd-overlay'+ name)[0].hide;
                if (typeof tmp_hide === 'function') tmp_hide();
            } else {
                $('#wd-overlay'+ name).remove();
            }
            return $(this);
        }
        ...
        // append
        $('body').append(
            '<div id="wd-overlay'+ name +'" style="display: none; left: 0px; top: 0px; '+ options.overlayStyle +'"'+
            '        class="wd-reset wd-overlay '+ ($(this).parents('.wd-popup, .wd-overlay-popup').length > 0 ? 'wd-overlay-popup' : '') +'">'+
            '    <style></style>'+
            '    <div style="min-width: 100%; '+ options.style +'" class="'+ options['class'] +'"></div>'+
            '</div>'
        );
        // init
        var div1 = $('#wd-overlay'+ name);
        ...
        div1.data('element', obj.length > 0 ? obj[0] : null)
            ...
            .on('mousedown', function (event) {
                $('#wd-overlay'+ name).data('keepOpen', true);
        ...
        function resize () {
            var div1 = $('#wd-overlay'+ name);
            var div2 = div1.find(' > div');
            var menu = $('#wd-overlay'+ name +' div.menu');
            ...
            if (div1.length > 0) {
                ...
                if ((maxHeight > -50 && maxHeight < 210) || options.openAbove === true) {
                    ...
                    div1.find('>style').html(
                        '#wd-overlay'+ name +':before { margin-left: '+ parseInt(tipLeft) +'px; }'+
                        '#wd-overlay'+ name +':after { margin-left: '+ parseInt(tipLeft) +'px; }'
                    );
                } else {
                    // show under
                    if (options.maxHeight && maxHeight > options.maxHeight) maxHeight = options.maxHeight;
                ...
                if (options.contextMenu || options.noTip) { // context menu
                    div1.find('>style').html(
                        '#wd-overlay'+ name +':before { display: none; }'+
                        '#wd-overlay'+ name +':after { display: none; }'
                    );
                }
        ...
        */
        ...
        if (menu === 'refresh') {
            ...
            if ($('#wd-overlay'+ name).length > 0) {
                ...
                var scrTop = $('#wd-overlay'+ name +' div.menu').scrollTop();
                $('#wd-overlay'+ name +' div.menu').html(getMenuHTML());
                $('#wd-overlay'+ name +' div.menu').scrollTop(scrTop);
                mresize();
            } else {
                $(this).w2menu(options);
            }
            ...
            $menu.find('tr.wd-selected').removeClass('wd-selected'); // clear all
            cur.addClass('wd-selected'); // select current
            // scroll into view
            if (cur.length > 0) {
            ...
            $.fn.w2menuClick = function (event, index) {
                if (['radio', 'check'].indexOf(options.type) != -1) {
                    // move checkbox
                    $(event.target).parents('tr').find('.wd-icon')
                        .removeClass('wd-icon-empty')
                        .addClass('wd-icon-check');
                }
            ...
            $.fn.w2menuDown = function (event, index) {
                ...
                var tmp = $el.find('.wd-icon');
                if (tmp.hasClass('wd-icon-empty')) {
                    if (options.type == 'radio') {
                        tmp.parents('table').find('.wd-icon')
                            .removeClass('wd-icon-check')
                            .addClass('wd-icon-empty');
                    }
                    tmp.removeClass('wd-icon-empty').addClass('wd-icon-check');
                } else if (tmp.hasClass('wd-icon-check') && (options.type == 'check')) {
                    tmp.removeClass('wd-icon-check').addClass('wd-icon-empty');
                }
                $el.parent().find('tr').removeClass('wd-selected');
                $el.addClass('wd-selected');
                $.fn.w2menuTmp = $el;
            };
            $.fn.w2menuOut = function (event, index) {
                var $tmp = $($.fn.w2menuTmp);
                if ($tmp.length > 0) {
                    $tmp.removeClass('wd-selected');
                    $tmp.find('.wd-icon').removeClass('wd-icon-check');
                    delete $.fn.w2menuTmp;
                }
            };
            var html = '';
            if (options.search) {
                html +=
                    ...
                    '    <div class="wd-icon icon-search" style="position: absolute; margin-top: 4px; margin-left: 6px; width: 11px; background-position: left !important;"></div>'+
            ...
            setTimeout(function () {
                ...
                if (options.search) {
                    ...
                    $('#wd-overlay'+ name +' #menu-search').focus();
                }
                mresize();
            }, 200);
            mresize();
        ...
        function mresize() {
            setTimeout(function () {
                ...
                var cur    = $('#wd-overlay'+ name +' tr[index='+ options.index +']');
                var scrTop = $('#wd-overlay'+ name +' div.menu').scrollTop();
                cur.addClass('wd-selected');
                if (options.tmp) options.tmp.contentHeight = $('#wd-overlay'+ name +' table').height() + (options.search ? 50 : 10);
                if (options.tmp) options.tmp.contentWidth  = $('#wd-overlay'+ name +' table').width();
                if ($('#wd-overlay'+ name).length > 0) $('#wd-overlay'+ name)[0].resize();
                // scroll into view
                if (cur.length > 0) {
                    var top    = cur[0].offsetTop - 5; // 5 is margin top
                    var el     = $('#wd-overlay'+ name +' div.menu');
                    var height = el.height();
                    $('#wd-overlay'+ name +' div.menu').scrollTop(scrTop);
                    if (top < scrTop || top + cur.height() > scrTop + height) {
                        $('#wd-overlay'+ name +' div.menu').animate({ 'scrollTop': top - (height - cur.height() * 2) / 2 }, 200, 'linear');
                    }
                }
            }, 1);
        }
        ...
        function getMenuHTML() {
            if (options.spinner) {
                return  '<table class="wd-drop-menu"><tbody><tr><td style="padding: 5px 10px 10px 10px; text-align: center">'+
                        '    <div class="wd-spinner" style="width: 18px; height: 18px; position: relative; top: 5px;"></div> '+
                        '    <div style="display: inline-block; padding: 3px; color: #999;">'+ w2utils.lang('Loading...') +'</div>'+
                        '</td></tr></tbody></table>';
            }
            var count        = 0;
            var menu_html    = '<table cellspacing="0" cellpadding="0" class="wd-drop-menu"><tbody>';
            ...
            for (var f = 0; f < options.items.length; f++) {
                ...
                if (['radio', 'check'].indexOf(options.type) != -1) {
                    if (mitem.checked === true) icon = 'wd-icon-check'; else icon = 'wd-icon-empty';
                }
                if (mitem.hidden !== true) {
                    var imgd = '';
                    var txt = mitem.text;
                    if (typeof options.render === 'function') txt = options.render(mitem, options);
                    if (img)  imgd = '<td class="menu-icon"><div class="wd-tb-image wd-icon '+ img +'"></div></td>';
                    if (icon) imgd = '<td class="menu-icon" align="center"><span class="wd-icon '+ icon +'"></span></td>';
                    // render only if non-empty
                    if (txt != null && txt !== '' && !(/^-+$/.test(txt))) {
                        var bg = (count % 2 === 0 ? 'wd-item-even' : 'wd-item-odd');
                        ...
                        menu_html +=
                            ...
                            '        class="'+ bg +' '+ (options.index === f ? 'wd-selected' : '') + ' ' + (mitem.disabled === true ? 'wd-disabled' : '') +'"'+
    ...
    $.fn.w2color = function (options, callBack) {
        ...
        if ($('#wd-overlay').length == 0) {
            ...
            $('#wd-overlay .wd-color').parent().html(getColorHTML(options));
        }
        // bind events
        $('#wd-overlay .color')
            .on('mousedown', function (event) {
            ...
            .on('mouseup', function () {
                setTimeout(function () {
                    if ($("#wd-overlay").length > 0) $('#wd-overlay').removeData('keepOpen')[0].hide();
                }, 10);
            });
        $('#wd-overlay input')
            .on('mousedown', function (event) {
                $('#wd-overlay').data('keepOpen', true);
                setTimeout(function () { $('#wd-overlay').data('keepOpen', true); }, 10);
            ...
            .on('change', function (event) {
                ...
                setTimeout(function() { $('#wd-overlay input')[0].focus(); }, 100);
            })
            .w2field('hex');
...
(function ($) {
    ...
    $.fn.w2grid = function(method) {
        if ($.isPlainObject(method)) {
            ...
            // register new object
            wd[object.name] = object;
            return object;

        } else {
            var obj = wd[$(this).attr('name')];
            if (!obj) return null;
    ...
    w2grid.prototype = {
        ...
        buttons: {
            'reload'   : { type: 'button', id: 'wd-reload', icon: 'wd-icon-reload', tooltip: 'Reload data in the list' },
            'columns'  : { type: 'drop', id: 'wd-column-on-off', icon: 'wd-icon-columns', tooltip: 'Show/hide columns', arrow: false, html: '' },
            'search'   : { type: 'html',   id: 'wd-search',
                            html: '<div class="wd-icon icon-search-down wd-search-down" title="'+ w2utils.lang('Select Search Field') +'" '+
                                  'onclick="var obj = wd[$(this).parents(\'div.wd-grid\').attr(\'name\')]; obj.searchShowFields();"></div>'
                          },
            'search-go': { type: 'drop',  id: 'wd-search-advanced', icon: 'wd-icon-search', text: 'Search', tooltip: 'Open Search Fields' },
            'add'      : { type: 'button', id: 'wd-add', text: 'Add New', tooltip: 'Add new record', icon: 'wd-icon-plus' },
            'edit'     : { type: 'button', id: 'wd-edit', text: 'Edit', tooltip: 'Edit selected record', icon: 'wd-icon-pencil', disabled: true },
            'delete'   : { type: 'button', id: 'wd-delete', text: 'Delete', tooltip: 'Delete selected records', icon: 'wd-icon-cross', disabled: true },
            'save'     : { type: 'button', id: 'wd-save', text: 'Save', tooltip: 'Save changed records', icon: 'wd-icon-check' }
        },
        ...
        refreshRanges: function () {
            ...
            for (var i = 0; i < this.ranges.length; i++) {
                ...
                if (td1f.length > 0 || td2f.length > 0) {
                    if ($range.length == 0) {
                        rec1.append('<div id="grid_'+ this.name +'_f' + rg.name +'" class="wd-selection" style="'+ rg.style +'">'+
                                        (rg.name == 'selection' ?  '<div id="grid_'+ this.name +'_resizer" class="wd-selection-resizer"></div>' : '')+
                                    '</div>');
                        $range = $('#grid_'+ this.name +'_f'+ rg.name);
                    } else {
                        $range.attr('style', rg.style);
                        $range.find('.wd-selection-resizer').show();
                    }
                    if (td2f.length == 0) {
                        ...
                        $range.find('.wd-selection-resizer').hide();
                    }
                ...
                if (td1.length > 0 || td2.length > 0) {
                    if ($range.length == 0) {
                        rec2.append('<div id="grid_'+ this.name +'_' + rg.name +'" class="wd-selection" style="'+ rg.style +'">'+
                                        (rg.name == 'selection' ?  '<div id="grid_'+ this.name +'_resizer" class="wd-selection-resizer"></div>' : '')+
                                    '</div>');
            ...
            // add resizer events
            $(this.box).find('.wd-selection-resizer')
            ...
            if (this.selectType == 'row') {
                for (var a = 0; a < arguments.length; a++) {
                    ...
                    if (this.selectType == 'row') {
                        ...
                        if (recEl1 && recEl2) {
                            recEl1.addClass('wd-selected').data('selected', 'yes').find('.wd-col-number').addClass('wd-row-selected');
                            recEl2.addClass('wd-selected').data('selected', 'yes').find('.wd-col-number').addClass('wd-row-selected');
                            recEl1.find('.wd-grid-select-check').prop("checked", true);
                        }
                        selected++;
                    }
                }
                ...
                for (var recid in new_sel) {
                    ...
                    for (var t = 0; t < new_sel[recid].length; t++) {
                        ...
                        if (recEl1) {
                            recEl1.find('#grid_'+ this.name +'_data_'+ index +'_'+ col).addClass('wd-selected');
                            recEl1.find('.wd-col-number').addClass('wd-row-selected');
                            recEl1.data('selected', 'yes');
                            recEl1.find('.wd-grid-select-check').prop("checked", true);
                        }
                        if (recEl2) {
                            recEl2.find('#grid_'+ this.name +'_data_'+ index +'_'+ col).addClass('wd-selected');
                            recEl2.find('.wd-col-number').addClass('wd-row-selected');
                            recEl2.data('selected', 'yes');
                            recEl2.find('.wd-grid-select-check').prop("checked", true);
                        }
                        selected++;
                    }
                ...
                for (var c = 0; c < col_sel.length; c++) {
                    $(this.box).find('#grid_'+ this.name +'_column_'+ col_sel[c] +' .wd-col-header').addClass('wd-col-selected');
                }
            }
        ...
        unselect: function () {
            ...
            for (var a = 0; a < arguments.length; a++) {
                ...
                if (this.selectType == 'row') {
                    ...
                    recEl1.removeClass('wd-selected wd-inactive').removeData('selected').find('.wd-col-number').removeClass('wd-row-selected');
                    recEl2.removeClass('wd-selected wd-inactive').removeData('selected').find('.wd-col-number').removeClass('wd-row-selected');
                    if (recEl1.length != 0) {
                    ...
                    recEl1.find('.wd-grid-select-check').prop("checked", false);
                    unselected++;
                } else {
                    ...
                    $('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(recid)).find(' > td[col='+ col +']').removeClass('wd-selected wd-inactive');
                    $('#grid_'+ this.name +'_frec_'+ w2utils.escapeId(recid)).find(' > td[col='+ col +']').removeClass('wd-selected wd-inactive');
                    ...
                    if (!isColSelected) {
                       $(this.box).find('.wd-grid-columns td[col='+ col +'] .wd-col-header, .wd-grid-fcolumns td[col='+ col +'] .wd-col-header').removeClass('wd-col-selected');
                    }
                    if (!isRowSelected) {
                        $('#grid_'+ this.name +'_frec_'+ w2utils.escapeId(recid)).find('.wd-col-number').removeClass('wd-row-selected');
                    }
                    unselected++;
                    if (s.length == 0) {
        ...
        selectAll: function () {
            ...
            // add selected class
            if (this.selectType == 'row') {
                $(this.box).find('.wd-grid-records tr').not('.wd-empty-record')
                    .addClass('wd-selected').data('selected', 'yes').find('.wd-col-number').addClass('wd-row-selected');
                $(this.box).find('.wd-grid-frecords tr').not('.wd-empty-record')
                    .addClass('wd-selected').data('selected', 'yes').find('.wd-col-number').addClass('wd-row-selected');
                $(this.box).find('input.wd-grid-select-check').prop('checked', true);
            } else {
                $(this.box).find('.wd-grid-columns td .wd-col-header, .wd-grid-fcolumns td .wd-col-header').addClass('wd-col-selected');
                $(this.box).find('.wd-grid-records tr .wd-col-number').addClass('wd-row-selected');
                $(this.box).find('.wd-grid-records tr').not('.wd-empty-record')
                    .find('.wd-grid-data').not('.wd-col-select').addClass('wd-selected').data('selected', 'yes');
                $(this.box).find('.wd-grid-frecords tr .wd-col-number').addClass('wd-row-selected');
                $(this.box).find('.wd-grid-frecords tr').not('.wd-empty-record')
                    .find('.wd-grid-data').not('.wd-col-select').addClass('wd-selected').data('selected', 'yes');
                $(this.box).find('input.wd-grid-select-check').prop('checked', true);
            }
            ...
            var sel = this.getSelection();
            if (sel.length == 1) this.toolbar.enable('wd-edit'); else this.toolbar.disable('wd-edit');
            if (sel.length >= 1) this.toolbar.enable('wd-delete'); else this.toolbar.disable('wd-delete');
            this.addRange('selection');
            $('#grid_'+ this.name +'_check_all').prop('checked', true);
            // event after
            this.trigger($.extend(eventData, { phase: 'after' }));
        ...
        selectNone: function () {
            ...
            if (this.selectType == 'row') {
                $(this.box).find('.wd-grid-records tr.wd-selected').removeClass('wd-selected wd-inactive').removeData('selected')
                    .find('.wd-col-number').removeClass('wd-row-selected');
                $(this.box).find('.wd-grid-frecords tr.wd-selected').removeClass('wd-selected wd-inactive').removeData('selected')
                    .find('.wd-col-number').removeClass('wd-row-selected');
                $(this.box).find('input.wd-grid-select-check').prop('checked', false);
            } else {
                $(this.box).find('.wd-grid-columns td .wd-col-header, .wd-grid-fcolumns td .wd-col-header').removeClass('wd-col-selected');
                $(this.box).find('.wd-grid-records tr .wd-col-number').removeClass('wd-row-selected');
                $(this.box).find('.wd-grid-frecords tr .wd-col-number').removeClass('wd-row-selected');
                $(this.box).find('.wd-grid-data.wd-selected').removeClass('wd-selected wd-inactive').removeData('selected');
                $(this.box).find('input.wd-grid-select-check').prop('checked', false);
            }
            sel.indexes = [];
            sel.columns = {};
            this.toolbar.disable('wd-edit', 'wd-delete');
            this.removeRange('selection');
            $('#grid_'+ this.name +'_check_all').prop('checked', false);
            // event after
            this.trigger($.extend(eventData, { phase: 'after' }));
        ...
        searchOpen: function () {
            ...
            var obj = this;
            // show search
            $('#tb_'+ this.name +'_toolbar_item_wd-search-advanced').w2overlay({
                html    : this.getSearchesHTML(),
                name    : this.name + '-searchOverlay',
                left    : -10,
                'class' : 'wd-grid-searches',
                onShow  : function () {
                    obj.initSearches();
                    $('#wd-overlay-'+ obj.name +'-searchOverlay .wd-grid-searches').data('grid-name', obj.name);
                    var sfields = $('#wd-overlay-'+ this.name +'-searchOverlay .wd-grid-searches *[rel=search]');
        ...
        searchClose: function () {
            if (!this.box) return;
            if (this.searches.length == 0) return;
            if (this.toolbar) this.toolbar.uncheck('wd-search-advanced');
            // hide search
        ...
        searchShowFields: function () {
            var obj  = this;
            var el   = $('#grid_'+ this.name +'_search_all');
            var html = '<div class="wd-select-field"><table><tbody>';
            for (var s = -1; s < this.searches.length; s++) {
                ...
                html += '<tr '+ (w2utils.isIOS ? 'onTouchStart' : 'onClick') +'="wd[\''+ this.name +'\'].initAllField(\''+ search.field +'\');'+
        ...
        request: function (cmd, add_params, url, callBack) {
            ...
            if (this.last.xhr_offset == 0) {
                ...
                if (this.autoLoad === true) {
                    more.show().find('td').html('<div><div style="width: 20px; height: 20px;" class="wd-spinner"></div></div>');
                } else {
        ...
        getChanges: function () {
            ...
            for (var r = 0; r < this.records.length; r++) {
                ...
                if (rec.wd && rec.wd.changes != null) {
                    changes.push($.extend(true, { recid: rec.recid }, rec.wd.changes));
                }
            }
            return changes;
        },
        ...
        mergeChanges: function () {
            ...
            for (var c = 0; c < changes.length; c++) {
                ...
                for (var s in changes[c]) {
                    ...
                    if (record.wd) delete record.wd.changes;
                }
            }
            this.refresh();
        },
        ...
        editField: function (recid, column, value, event) {
            ...
            var edit   = (rec && rec.wd ? rec.wd.editable : null);
            if (edit == null || edit === true) edit = (col ? col.editable : null);
            if (!rec || !col || !edit || (rec.wd && rec.wd.editable === false)) return;
            ...
            var val = (rec.wd && rec.wd.changes && rec.wd.changes[col.field] != null ? w2utils.stripTags(rec.wd.changes[col.field]) : w2utils.stripTags(rec[col.field]));
            if (val == null) val = '';
            ...
            if (edit.type == 'select') {
                ...
                el.addClass('wd-editable')
                    .html('<select id="grid_'+ obj.name +'_edit_'+ recid +'_'+ column +'" column="'+ column +'" class="wd-input"'+
                ...
                el.addClass('wd-editable')
                    .html('<div id="grid_'+ obj.name +'_edit_'+ recid +'_'+ column +'" class="wd-input"'+
                        ...
                        '></div>' + edit.outTag);
                if (value == null) el.find('div.wd-input').text(typeof val != 'object' ? val : '');
                // add blur listener
                var input = el.find('div.wd-input').get(0);
                setTimeout(function () {
                    var tmp = input;
                ...
                el.addClass('wd-editable')
                    .html('<input id="grid_'+ obj.name +'_edit_'+ recid +'_'+ column +'" autocorrect="off" autocomplete="off" spellcheck="false" type="text" '+
                        ...
                        '    field="'+ col.field +'" recid="'+ recid +'" column="'+ column +'" class="wd-input"'+ edit.inTag +
                        '/>' + edit.outTag);
                // issue #499
                if (edit.type == 'number') {
            ...
            setTimeout(function () {
                el.find('input, select, div.wd-input')
                    ...
                    .on('click', function (event) {
                        if (edit.type == 'div') {
                            expand.call(el.find('div.wd-input')[0], null);
                        } else {
                        ...
                        setTimeout(function () {
                            switch (event.keyCode) {
                                ...
                                case 27: // escape
                                    ...
                                    if (rec.wd && rec.wd.changes && rec.wd.changes[col.field] != null) old = rec.wd.changes[col.field];
                ...
                // focus and select
                if (edit.type == 'div') {
                    var tmp = el.find('div.wd-input').focus();
                ...
                // event after
                obj.trigger($.extend(eventData, { phase: 'after', input: el.find('input, select, div.wd-input') }));
            }, 1);
            return;
        ...
        editChange: function (el, index, column, event) {
            ...
            if (el.type == 'checkbox') {
                if (rec.wd && rec.wd.editable === false) el.checked = !el.checked;
                new_val = el.checked;
            }
            ...
            var eventData = {
                ...
                value_new: new_val,
                value_previous: (rec.wd && rec.wd.changes && rec.wd.changes.hasOwnProperty(col.field) ? rec.wd.changes[col.field]: old_val),
            ...
            while (true) {
                ...
                if ((typeof new_val != 'object' && String(old_val) != String(new_val)) ||
                    ...
                    if (eventData.isCancelled !== true) {
                        ...
                        // default action
                        rec.wd = rec.wd || {};
                        rec.wd.changes = rec.wd.changes || {};
                        rec.wd.changes[col.field] = eventData.value_new;
                        // event after
                        this.trigger($.extend(eventData, { phase: 'after' }));
                    }
                } else {
                    ...
                    if (eventData.isCancelled !== true) {
                        ...
                        // default action
                        if (rec.wd && rec.wd.changes) delete rec.wd.changes[col.field];
                        if (rec.wd && $.isEmptyObject(rec.wd.changes)) delete rec.wd.changes;
                        // event after
                        this.trigger($.extend(eventData, { phase: 'after' }));
                    }
                }
            ...
            if (!summary) {
                if (rec.wd && rec.wd.changes && rec.wd.changes[col.field] != null) {
                    cell.addClass('wd-changed');
                } else {
                    cell.removeClass('wd-changed');
                }
                // update cell data
            ...
            if (this.show.toolbarSave) {
                if (this.getChanges().length > 0) this.toolbar.enable('wd-save'); else this.toolbar.disable('wd-save');
            }
        },
        ...
        "delete": function (force) {
            ...
            if (this.msgDelete != '' && !force) {
                w2confirm({
                    ...
                    yes_class : 'wd-btn-red',
                    callBack: function (result) {
                        if (result == 'Yes') wd[obj.name]['delete'](true);
                    }
                });
                return;
            }
                ...
                if (typeof recs[0] != 'object') {
                    ...
                    for (var r = 0; r < recs.length; r++) {
                        ...
                        if (ind != null && fld != 'recid') {
                            ...
                            if (rec.wd && rec.wd.changes) delete rec.wd.changes[fld];
        ...
        focus: function (event) {
            ...
            // default behaviour
            this.hasFocus = true;
            $(this.box).find('.wd-inactive').removeClass('wd-inactive');
            setTimeout(function () {
                var $input = $(obj.box).find('#grid_'+ obj.name + '_focus');
                if (!$input.is(':focus')) $input.focus();
            }, 10);
        ...
        blur: function (event) {
            ...
            // default behaviour
            this.hasFocus = false;
            $(this.box).find('.wd-selected').addClass('wd-inactive');
            $(this.box).find('.wd-selection').addClass('wd-inactive');
            // event after
            this.trigger($.extend(eventData, { phase: 'after' }));
        },
        ...
        dblClick: function (recid, event) {
            ...
            if (col && $.isPlainObject(col.editable)) {
                ...
                this.select({ recid: recid, column: column });
                if (this.show.expandColumn || (rec.wd && Array.isArray(rec.wd.children))) this.toggle(recid);
            }
            // event after
            this.trigger($.extend(eventData, { phase: 'after' }));
        },
        ...
        toggle: function (recid) {
            var rec = this.get(recid);
            rec.wd = rec.wd || {};
            if (rec.wd.expanded === true) return this.collapse(recid); else return this.expand(recid);
        },
        ...
        expand: function (recid) {
            ...
            var children = rec.wd.children;
            if (Array.isArray(children)) {
                if (rec.wd.expanded === true || children.length == 0) return false; // already shown
                var edata = this.trigger({ phase: 'before', type: 'expand', target: this.name, recid: recid });
                if (edata.isCancelled === true) return false;
                rec.wd.expanded = true;
                children.forEach(function (child) {
                    child.wd = child.wd || {};
                    child.wd.parent_recid = rec.recid;
                    if (child.wd.children == null) child.wd.children = [];
                });
                ...
                if (rec.wd.expanded == 'none') return false;
                // insert expand row
                $('#grid_'+ this.name +'_rec_'+ id).after(
                        '<tr id="grid_'+ this.name +'_rec_'+ recid +'_expanded_row" class="wd-expanded-row">'+
                        '    <td colspan="100" class="wd-expanded2">'+
                        '        <div id="grid_'+ this.name +'_rec_'+ recid +'_expanded"></div>'+
                        '    </td>'+
                        '    <td class="wd-grid-data-last"></td>'+
                        '</tr>');

                $('#grid_'+ this.name +'_frec_'+ id).after(
                        '<tr id="grid_'+ this.name +'_frec_'+ recid +'_expanded_row" class="wd-expanded-row">'+
                            (this.show.lineNumbers ? '<td class="wd-col-number"></td>' : '') +
                        '    <td class="wd-grid-data wd-expanded1" colspan="100">'+
                ...
                // default action
                $('#grid_'+ this.name +'_rec_'+ id).attr('expanded', 'yes').addClass('wd-expanded');
                $('#grid_'+ this.name +'_frec_'+ id).attr('expanded', 'yes').addClass('wd-expanded');
        ...
        collapse: function (recid) {
            ...
            var children = rec.wd.children;
            if (Array.isArray(children)) {
                if (rec.wd.expanded !== true) return false; // already hidden
                var edata = this.trigger({ phase: 'before', type: 'collapse', target: this.name, recid: recid });
                if (edata.isCancelled === true) return false;
                rec.wd.expanded = false;
                // all children are directly after
                var start = ind + 1;
                var end   = start;
                while (true) {
                    if (this.records.length <= end + 1 || this.records[end+1].wd == null || this.records[end+1].wd.parent_recid == null) break;
                    if (this.records[end+1].wd.expanded) this.records[end+1].wd.expanded = false;
                    end++;
                }
                ...
                if (edata.isCancelled === true) return false;
                // default action
                $('#grid_'+ this.name +'_rec_'+ id).removeAttr('expanded').removeClass('wd-expanded');
                $('#grid_'+ this.name +'_frec_'+ id).removeAttr('expanded').removeClass('wd-expanded');
                $('#grid_'+ this.name +'_cell_'+ this.get(recid, true) +'_expand div').html('+');
        ...
        resize: function () {
            ...
            $(this.box).find('> div:not(.wd-lock-msg)')
            ...
            if (cells == null) {
                for (var index = this.last.range_start - 1; index <= this.last.range_end - 1; index++) {
                    ...
                    for (var column = 0; column < this.columns.length; column++) {
                        ...
                        // assign style
                        if (rec.wd && rec.wd.style != null && !$.isEmptyObject(rec.wd.style)) {
                            if (typeof rec.wd.style == 'string') {
                                $(this.box).find('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(rec.recid)).attr('style', rec.wd.style);
                            }
                            if ($.isPlainObject(rec.wd.style) && typeof rec.wd.style[column] == 'string') {
                                cell.attr('style', rec.wd.style[column]);
                            }
                        } else {
                            cell.attr('style', '');
                        }
        ...
        refreshCell: function (recid, field) {
            ...
            if (rec.wd && rec.wd.style != null && !$.isEmptyObject(rec.wd.style)) {
                ...
                if ($.isPlainObject(rec.wd.style) && typeof rec.wd.style[col_ind] == 'string') {
                    cell.attr('style', rec.wd.style[col_ind]);
                }
            } else {
                cell.attr('style', '');
            }
        ...
        refreshRow: function (recid, ind) {
            ...
            if (tr1.length > 0) {
                ...
                var st = this.records[ind].wd.style;
                if (typeof st == 'string') {
                    var tr1 = $(this.box).find('#grid_'+ this.name +'_frec_'+ w2utils.escapeId(recid));
                    var tr2 = $(this.box).find('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(recid));
                    tr1.attr('custom_style', st);
                    tr2.attr('custom_style', st);
                    if (!tr1.hasClass('wd-selected')) {
        ...
        refresh: function () {
            ...
            if (this.show.toolbar) {
                ...
                if (this.toolbar && this.toolbar.get('wd-column-on-off') && this.toolbar.get('wd-column-on-off').checked) {
                    // no action
                } else {
                    ...
                    if (typeof this.toolbar == 'object') {
                        ...
                        for (var t = 0; t < tmp.length; t++) {
                            if (tmp[t].id == 'wd-search' || tmp[t].type == 'break') continue;
            ...
            // -- separate summary
            var tmp = this.find({ 'wd.summary': true }, true);
            if (tmp.length > 0) {
                this.summary = [];
            ...
            var bodyHTML =
                '<div id="grid_'+ this.name +'_frecords" class="wd-grid-frecords" style="margin-bottom: '+ (w2utils.scrollBarSize() - 1) +'px;">'+
                    recHTML[0] +
                '</div>'+
                '<div id="grid_'+ this.name +'_records" class="wd-grid-records" onscroll="wd[\''+ this.name +'\'].scroll(event);">' +
                    recHTML[1] +
                '</div>'+
                '<div id="grid_'+ this.name +'_scroll1" class="wd-grid-scroll1" style="height: '+ w2utils.scrollBarSize() +'px"></div>'+
                // Columns need to be after to be able to overlap
                '<div id="grid_'+ this.name +'_fcolumns" class="wd-grid-fcolumns">'+
                '    <table><tbody>'+ colHTML[0] +'</tbody></table>'+
                '</div>'+
                '<div id="grid_'+ this.name +'_columns" class="wd-grid-columns">'+
            ...
            var rows = obj.find({ 'wd.expanded': true }, true);
            for (var r = 0; r < rows.length; r++) {
                var tmp = obj.records[rows[r]].wd;
            ...
            if (obj.markSearch) {
                setTimeout(function () {
                    ...
                    if (str.length > 0) $(obj.box).find('.wd-grid-data > div').w2marker(str);
                }, 50);
            }
            // enable/disable toolbar search button
            if (this.show.toolbarSave) {
        ...
        render: function (box) {
            ...
            var grid_header_css = 'wd-grid-header';
            if (this.bordered == true) {
                $(this.box).addClass('wd-grid-bordered');
                grid_header_css = 'wd-grid-header-bordered';
            }
            ...
            $(this.box)
                .attr('name', this.name)
                .addClass('wd-reset wd-grid')
                .html('<div>'+
                      '    <div id="grid_'+ this.name +'_header" class="'+grid_header_css+'"></div>'+
                      '    <div id="grid_'+ this.name +'_toolbar" class="wd-grid-toolbar"></div>'+
                      '    <div id="grid_'+ this.name +'_body" class="wd-grid-body"></div>'+
                      '    <div id="grid_'+ this.name +'_fsummary" class="wd-grid-body wd-grid-summary"></div>'+
                      '    <div id="grid_'+ this.name +'_summary" class="wd-grid-body wd-grid-summary"></div>'+
                      '    <div id="grid_'+ this.name +'_footer" class="wd-grid-footer"></div>'+
                      '    <textarea id="grid_'+ this.name +'_focus" style="position: absolute; top: 0; right: 0; z-index: -1; opacity: 0"></textarea>'+
                      '</div>');
            if (this.selectType != 'row') $(this.box).addClass('wd-ss');
            ...
            $(this.box).find('#grid_'+ this.name + '_focus')
                ...
                .on('paste', function (event) {
                    var el = this;
                    setTimeout(function () { wd[obj.name].paste(el.value); el.value = ''; }, 1)
                })
                .on('keydown', function (event) {
                    wd[obj.name].keydown.call(wd[obj.name], event);
                });
            ...
            if ($('.wd-layout').length == 0) { // if there is layout, it will send a resize event
                this.tmp_resize = function (event) { wd[obj.name].resize(); };
            ...
            function mouseStart (event) {
                ...
                if (obj.last.userSelect == 'text') {
                    ...
                    $(obj.box).find('.wd-grid-body').css(w2utils.cssPrefix('user-select', 'none'));
                }
                // regular record select
                if (obj.selectType == 'row' && ($(event.target).parents().hasClass('wd-head') || $(event.target).hasClass('wd-head'))) return;
                if (obj.last.move && obj.last.move.type == 'expand') return;
                // if altKey - alow text selection
                if (event.altKey) {
                    $(obj.box).find('.wd-grid-body').css(w2utils.cssPrefix('user-select', 'text'));
                    obj.selectNone();
                    ...
                    var tmps = false;
                    while (tmp) {
                        if (!tmp || tmp.classList.contains('wd-grid')) break;
                ...
                if (obj.reorderRows == true) {
                    ...
                    if ($(el).hasClass('wd-col-number')) {
                        obj.selectNone();
                        obj.last.move.reorder = true;
                        // supress hover
                        var eColor = $(obj.box).find('.wd-even.wd-empty-record').css('background-color');
                        var oColor = $(obj.box).find('.wd-odd.wd-empty-record').css('background-color');
                        $(obj.box).find('.wd-even td').not('.wd-col-number').css('background-color', eColor);
                        $(obj.box).find('.wd-odd td').not('.wd-col-number').css('background-color', oColor);
                        ...
                        if (!mv.ghost) {
                            ...
                            var recs = $(obj.box).find('.wd-grid-records');
            ...
            function mouseMove (event) {
                ...
                if (recid == null) {
                    ...
                    if (isNaN(col)) {
                        ...
                        $(obj.box).find('.wd-grid-columns .wd-col-header, .wd-grid-fcolumns .wd-col-header').removeClass('wd-col-selected');
                        $(obj.box).find('.wd-col-number').removeClass('wd-row-selected');
                        delete mv.colRange;
                    } else {
                        ...
                        for (var j = parseInt(tmp[0]); j <= parseInt(tmp[1]); j++) {
                            $(obj.box).find('#grid_'+ obj.name +'_column_' + j + ' .wd-col-header').addClass('wd-col-selected');
                        }
                        $(obj.box).find('.wd-col-number').not('.wd-head').addClass('wd-row-selected');
                        // show new range
            ...
            function mouseStop (event) {
                ...
                if ($(event.target).parents().hasClass('.wd-head') || $(event.target).hasClass('.wd-head')) return;
        ...
        initColumnOnOff: function () {
            ...
            var obj = this;
            // line number
            var col_html = '<div class="wd-col-on-off">'+
                '<table><tbody>'+
                '<tr id="grid_'+ this.name +'_column_ln_check" onclick="wd[\''+ obj.name +'\'].columnOnOff(event, \'line-numbers\'); event.stopPropagation();">'+
                '   <td style="width: 30px; text-align: center; padding-right: 3px; color: #888;">'+
                '      <span class="wd-column-check wd-icon-'+ (!obj.show.lineNumbers ? 'empty' : 'check') +'"></span>'+
                '   </td>'+
                '   <td onclick="$(\'.wd-overlay\')[0].hide();">'+
            ...
            for (var c = 0; c < this.columns.length; c++) {
                ...
                col_html +=
                    ...
                    '       onclick="wd[\''+ obj.name +'\'].columnOnOff(event, \''+ col.field +'\'); event.stopPropagation();">'+
                    '   <td style="width: 30px; text-align: center; padding-right: 3px; color: #888;">'+
                    '      <span class="wd-column-check wd-icon-'+ (col.hidden ? 'empty' : 'check') +'"></span>'+
                    '   </td>'+
                    '   <td onclick="$(\'.wd-overlay\')[0].hide();">'+
            ...
            if (url && obj.show.skipRecords) {
                col_html +=
                        ...
                        '               wd[\''+ obj.name +'\'].skip(this.value); '+
                        '               $(\'.wd-overlay\')[0].hide(); '+
                        '            }"/> '+ w2utils.lang('Records')+
                        '    </div>'+
                        '</td></tr>';
            }
            col_html += '<tr><td colspan="2" onclick="var obj = wd[\''+ obj.name +'\']; obj.toolbar.uncheck(\'wd-column-on-off\'); obj.stateSave();">'+
                        '    <div style="cursor: pointer; padding: 4px 8px; cursor: default">'+ w2utils.lang('Save Grid State') + '</div>'+
                        '</td></tr>'+
                        '<tr><td colspan="2" onclick="var obj = wd[\''+ obj.name +'\']; obj.toolbar.uncheck(\'wd-column-on-off\'); obj.stateReset();">'+
                        '    <div style="cursor: pointer; padding: 4px 8px; cursor: default">'+ w2utils.lang('Restore Default State') + '</div>'+
                        '</td></tr>';
            col_html += "</tbody></table></div>";
            this.toolbar.get('wd-column-on-off').html = col_html;
        },
        ...
        initColumnDrag: function ( box ) {
            ...
            function dragColStart ( event ) {
                ...
                _dragData.timeout = setTimeout(function(){
                    ...
                    var eventData,
                        ...
                        origColumn,
                        origColumnNumber,
                        invalidPreColumns = [ 'wd-col-number', 'wd-col-expand', 'wd-col-select' ],
                        invalidPostColumns = [ 'wd-head-last' ],
                        invalidColumns = invalidPreColumns.concat( invalidPostColumns ),
                        preColumnsSelector = '.wd-col-number, .wd-col-expand, .wd-col-select',
                        preColHeadersSelector = '.wd-head.wd-col-number, .wd-head.wd-col-expand, .wd-head.wd-col-select';

                    // do nothing if it is not a header
                    if ( !$( event.originalEvent.target ).parents().hasClass( 'wd-head' ) ) return;
                    ...
                    _dragData.columnHead = origColumn = $( event.originalEvent.target ).parents( '.wd-head' );
                    ...
                    columns = _dragData.columns = $( obj.box ).find( '.wd-head:not(.wd-head-last)' );

                    //add events
                    $( document ).on( 'mouseup', dragColEnd );
                    $( document ).on( 'mousemove', dragColOver );

                    _dragData.originalPos = parseInt( $( event.originalEvent.target ).parent( '.wd-head' ).attr( 'col' ), 10 );
                    ...
                    if (this.bordered) {
                        $( _dragData.ghost ).find( '[col]:not([col="' + _dragData.originalPos + '"]), .wd-toolbar, .wd-grid-header-bordered' ).remove();
                    } else {
                        $( _dragData.ghost ).find( '[col]:not([col="' + _dragData.originalPos + '"]), .wd-toolbar, .wd-grid-header' ).remove();
                    }
                    $( _dragData.ghost ).find( preColumnsSelector ).remove();
                    $( _dragData.ghost ).find( '.wd-grid-body' ).css({ top: 0 });
                    ...
                    $( _dragData.ghost ).css({
                        ...
                        zIndex: 999999,
                        opacity: 0
                    }).addClass( '.wd-grid-ghost' ).animate({
                            width: selectedCol.width(),
                            height: $(obj.box).find('.wd-grid-body:first').height(),
                            left : event.pageX,
                            top : event.pageY,
                            opacity: .8
                        }, 0 );
            ...
            function dragColOver ( event ) {
                ...
                var cursorX = event.originalEvent.pageX,
                    ...
                    lastWidth = $( '.wd-head:not(.wd-head-last)' ).width();
            ...
            function markIntersection( intersection ){
                ...
                if ( !_dragData.lastInt || _dragData.lastInt !== intersection ){
                    ...
                    $('.wd-head').removeClass('wd-col-intersection');
                    ...
                    if ( intersection >= _dragData.columns.length ) {
                        ...
                        $( _dragData.columns[ _dragData.columns.length - 1 ] ).addClass('wd-col-intersection');
                        ...
                        $( _dragData.columns[ _dragData.numberPreColumnsPresent ] ).prev().addClass('wd-col-intersection');
                    } else {
                        ...
                        $( _dragData.columns[intersection - 1] ).addClass('wd-col-intersection');
                    }
                }
            }
            ...
            return {
                remove: function(){
                    ...
                    $( obj.box ).find( '.wd-head' ).removeAttr( 'draggable' );
        ...
        columnOnOff: function (event, field) {
            var $el = $(event.target).parents('tr').find('.wd-column-check');
            // event before
            ...
            var rows = obj.find({ 'wd.expanded': true }, true);
            for (var r = 0; r < rows.length; r++) {
                var tmp = this.records[r].wd;
                if (tmp && !Array.isArray(tmp.children)) {
                    this.records[r].wd.expanded = false;
                }
            }
            // show/hide
            if (field == 'line-numbers') {
                ...
                if (this.show.lineNumbers) {
                    $el.addClass('wd-icon-check').removeClass('wd-icon-empty');
                } else {
                    $el.addClass('wd-icon-empty').removeClass('wd-icon-check');
                }
                this.refresh();
            } else {
        ...
        initToolbar: function () {
            ...
            if (this.toolbar['render'] == null) {
                ...
                if (this.show.toolbarReload || this.show.toolbarColumns) {
                    this.toolbar.items.push({ type: 'break', id: 'wd-break0' });
                }
                if (this.show.toolbarInput) {
                    var html =
                        '<div class="wd-toolbar-search">'+
                        '<table cellpadding="0" cellspacing="0"><tbody><tr>'+
                        '    <td>'+ this.buttons['search'].html +'</td>'+
                        '    <td>'+
                        '        <input type="text" id="grid_'+ this.name +'_search_all" class="wd-search-all" tabindex="-1" '+
                        '            placeholder="'+ this.last.caption +'" value="'+ this.last.search +'"'+
                        '            onfocus="clearTimeout(wd[\''+ this.name +'\'].last.kbd_timer);"'+
                        '            onkeydown="if (event.keyCode == 13 &amp;&amp; w2utils.isIE) this.onchange();"'+
                        '            onchange="'+
                        '                var grid = wd[\''+ this.name +'\']; '+
                        '                var val = this.value; '+
                        ...
                        '    </td>'+
                        '    <td>'+
                        '        <div title="'+ w2utils.lang('Clear Search') +'" class="wd-search-clear" id="grid_'+ this.name +'_searchClear"  '+
                        '             onclick="var obj = wd[\''+ this.name +'\']; obj.searchReset();" style="display: none"'+
                        '        >&#160;&#160;</div>'+
                        '    </td>'+
                        '</tr></tbody></table>'+
                        '</div>';
                    this.toolbar.items.push({ type: 'html', id: 'wd-search', html: html });
                }
                ...
                if ((this.show.toolbarSearch || this.show.toolbarInput) && (this.show.toolbarAdd || this.show.toolbarEdit || this.show.toolbarDelete || this.show.toolbarSave)) {
                    this.toolbar.items.push({ type: 'break', id: 'wd-break1' });
                }
                ...
                if (this.show.toolbarSave) {
                    if (this.show.toolbarAdd || this.show.toolbarDelete || this.show.toolbarEdit) {
                        this.toolbar.items.push({ type: 'break', id: 'wd-break2' });
                    }
                ...
                this.toolbar.on('click', function (event) {
                    ...
                    switch (id) {
                        case 'wd-reload':
                            ...
                            obj.trigger($.extend(eventData2, { phase: 'after' }));
                            break;
                        case 'wd-column-on-off':
                            obj.initColumnOnOff();
                            obj.initResize();
                            obj.resize();
                            break;
                        case 'wd-search-advanced':
                            var tb = this;
                            var it = this.get(id);
                            if (it.checked) {
                                obj.searchClose();
                                ...
                                function tmp_close() {
                                    if ($('#wd-overlay-'+ obj.name + '-searchOverlay').data('keepOpen') === true) return;
                                    tb.uncheck(id);
                ...
                var pos1, pos2;
                var search = this.toolbar.get('wd-search');
                if (search != null) {
                    var tmp = search.html;
                    ...
                    setTimeout(function () {
                        obj.toolbar.set('wd-search', { html: tmp });
                    }, 1);
                }
            }
            return;
        ...
        resizeRecords: function () {
            ...
            $(this.box).find('.wd-empty-record').remove();
            ...
            if (bodyOverflowX || bodyOverflowY) {
                columns.find('> table > tbody > tr:nth-child(1) td.wd-head-last').css('width', w2utils.scrollBarSize()).show();
                records.css({
                ...
                columns.find('> table > tbody > tr:nth-child(1) td.wd-head-last').hide();
                records.css({
                    top: ((this.columnGroups.length > 0 && this.show.columns ? 1 : 0) + w2utils.getSize(columns, 'height')) +'px',
                    overflow: 'hidden'
                });
            ...
            if (this.show.emptyRecords && !bodyOverflowY) {
                ...
                if (this.fixedBody) {
                    for (var di = buffered; di <= max; di++) {
                        ...
                        var html2 = '';
                        var htmlp = '';
                        html1 += '<tr class="'+ (di % 2 ? 'wd-even' : 'wd-odd') + ' wd-empty-record" style="height: '+ this.recordHeight +'px">';
                        html2 += '<tr class="'+ (di % 2 ? 'wd-even' : 'wd-odd') + ' wd-empty-record" style="height: '+ this.recordHeight +'px">';
                        if (this.show.lineNumbers)  html1 += '<td class="wd-col-number"></td>';
                        if (this.show.selectColumn) html1 += '<td class="wd-grid-data wd-col-select"></td>';
                        if (this.show.expandColumn) html1 += '<td class="wd-grid-data wd-col-expand"></td>';
                        html2 += '<td class="wd-grid-data-spacer" col="start" style="border-right: 0"></td>';
                        for (var j = 0; j < this.columns.length; j++) {
                            var col = this.columns[j];
                            if ((col.hidden || j < this.last.colStart || j > this.last.colEnd) && !col.frozen) continue;
                            htmlp = '<td class="wd-grid-data" '+ (col.attr != null ? col.attr : '') +' col="'+ j +'"></td>';
                            if (col.frozen) html1 += htmlp; else html2 += htmlp;
                        }
                        html1 += '<td class="wd-grid-data-last"></td> </tr>';
                        html2 += '<td class="wd-grid-data-last" col="end"></td> </tr>';
            ...
            records.find('> table > tbody > tr:nth-child(1) td')
                ...
                .each(function (index, el) {
                    ...
                    // last column
                    if ($(el).hasClass('wd-grid-data-last') && $(el).parents('.wd-grid-frecords').length == 0) { // not in frecords
                        if (obj.last.colEnd + 1 < obj.columns.length) {
                            var width = 0;
                            for (var i = obj.last.colEnd + 1; i < obj.columns.length; i++) {
                                if (obj.columns[i].frozen || obj.columns[i].hidden) continue;
        ...
        getSearchesHTML: function () {
            ...
            for (var i = 0; i < this.searches.length; i++) {
                ...
                var btn = '';
                if (showBtn == false) {
                    btn = '<button class="wd-btn close-btn" onclick="obj = wd[\''+ this.name +'\']; if (obj) obj.searchClose()">X</button>';
                    showBtn = true;
                }
                ...
                if (['text', 'alphanumeric', 'combo'].indexOf(s.type) != -1) {
                    var operator =
                        '<select id="grid_'+ this.name +'_operator_'+ i +'" class="wd-input" onclick="event.stopPropagation();"' +
                        '   onchange="wd[\''+ this.name + '\'].initOperator(this, '+ i +')">' +
                            getOpearators('text') +
                        '</select>';
                }
                ...
                switch (s.type) {
                    ...
                    case 'combo':
                    case 'enum':
                        html += '<input rel="search" type="text" size="40" class="wd-input" style="'+ s.style +'" id="grid_'+ this.name +'_field_'+ i +'" name="'+ s.field +'" '+ s.inTag +'/>';
                        break;
                    ...
                    case 'date':
                    case 'time':
                        html += '<input rel="search" type="text" size="12" class="wd-input" style="'+ s.style +'" id="grid_'+ this.name +'_field_'+ i +'" name="'+ s.field +'" '+ s.inTag +'/>'+
                                '<span id="grid_'+ this.name +'_range_'+ i +'" style="display: none">'+
                                '&#160;-&#160;&#160;<input rel="search" type="text" class="wd-input" style="width: 90px" id="grid_'+ this.name +'_field2_'+i+'" name="'+ s.field +'" '+ s.inTag +'/>'+
                                '</span>';
                        break;

                    case 'select':
                        html += '<select rel="search" class="wd-input" style="'+ s.style +'" id="grid_'+ this.name +'_field_'+ i +'" '+
            ...
            html += '<tr>'+
                    ...
                    '        <div>'+
                    '        <button class="wd-btn" onclick="obj = wd[\''+ this.name +'\']; if (obj) { obj.searchReset(); }">'+ w2utils.lang('Reset') + '</button>'+
                    '        <button class="wd-btn wd-btn-blue" onclick="obj = wd[\''+ this.name +'\']; if (obj) { obj.search(); }">'+ w2utils.lang('Search') + '</button>'+
                    '        </div>'+
                    '    </td>'+
                    '</tr></tbody></table>';
            return html;
        ...
        initSearches: function () {
            ...
            // add on change event
            $('#wd-overlay-'+ this.name +'-searchOverlay .wd-grid-searches *[rel=search]').on('keypress', function (evnt) {
                if (evnt.keyCode == 13) {
                    obj.search();
        ...
        getColumnsHTML: function () {
            ...
            function getGroups () {
                ...
                if (obj.show.lineNumbers) {
                    html1 += '<td class="wd-head wd-col-number">'+
                            '    <div style="height: '+ (obj.recordHeight+1) +'px">&#160;</div>'+
                            '</td>';
                }
                if (obj.show.selectColumn) {
                    html1 += '<td class="wd-head wd-col-select">'+
                            '    <div style="height: 25px">&#160;</div>'+
                            '</td>';
                }
                if (obj.show.expandColumn) {
                    html1 += '<td class="wd-head wd-col-expand">'+
                            '    <div style="height: 25px">&#160;</div>'+
                            '</td>';
                }
                var ii = 0;
                html2 += '<td id="grid_'+ obj.name + '_column_start" class="wd-head" col="start" style="border-right: 0"></td>';
                    ...
                    if (colspan <= 0) {
                        ...
                        for (var si = 0; si < obj.sortData.length; si++) {
                            if (obj.sortData[si].field == col.field) {
                                if (new RegExp('asc', 'i').test(obj.sortData[si].direction))  sortStyle = 'wd-sort-up';
                                if (new RegExp('desc', 'i').test(obj.sortData[si].direction)) sortStyle = 'wd-sort-down';
                            }
                        }
                        var resizer = "";
                        if (col.resizable !== false) {
                            resizer = '<div class="wd-resizer" name="'+ ii +'"></div>';
                        }
                        tmpf = '<td id="grid_'+ obj.name + '_column_' + ii +'" class="wd-head '+ sortStyle +'" col="'+ ii + '" '+
                               '    rowspan="2" colspan="'+ colspan +'" '+
                               '    oncontextmenu = "wd[\''+ obj.name +'\'].contextMenu(null, '+ ii +', event);"'+
                               '    onclick="wd[\''+ obj.name +'\'].columnClick(\''+ col.field +'\', event);">'+
                                   resizer +
                               '    <div class="wd-col-group wd-col-header '+ (sortStyle ? 'wd-col-sorted' : '') +'">'+
                        ...
                        tmpf = '<td id="grid_'+ obj.name + '_column_' + ii +'" class="wd-head" col="'+ ii + '" '+
                               '        colspan="'+ colspan +'">'+
                               '    <div class="wd-col-group">'+
                ...
                html2 += '<td id="grid_'+ obj.name + '_column_end" class="wd-head" col="end"></td></tr>';
                return [html1, html2];
            }
            ...
            function getColumns (master) {
                ...
                var html2 = '<tr>';
                if (obj.show.lineNumbers) {
                    html1 += '<td class="wd-head wd-col-number" onclick="wd[\''+ obj.name +'\'].columnClick(\'line-number\', event);">'+
                            '    <div>#</div>'+
                            '</td>';
                }
                if (obj.show.selectColumn) {
                ...
                html1 += '<td class="wd-head wd-head-last"><div>&#160;</div></td>';
                html2 += '<td class="wd-head wd-head-last" col="end"><div>&#160;</div></td>';
                html1 += '</tr>';
                html2 += '</tr>';
                return [html1, html2];
            }
        ...
        getColumnCellHTML: function (i) {
            ...
            if (col == null) return '';
            // reorder style
            var reorderCols = (this.reorderColumns && (!this.columnGroups || !this.columnGroups.length)) ? ' wd-reorder-cols-head ' : '';
            // sort style
            var sortStyle = '';
            for (var si = 0; si < this.sortData.length; si++) {
                if (this.sortData[si].field == col.field) {
                    if (new RegExp('asc', 'i').test(this.sortData[si].direction))  sortStyle = 'wd-sort-up';
                    if (new RegExp('desc', 'i').test(this.sortData[si].direction)) sortStyle = 'wd-sort-down';
                }
            }
            // col selected
            ...
            var html = '<td id="grid_'+ this.name + '_column_' + i +'" col="'+ i +'" class="wd-head '+ sortStyle + reorderCols + '" ' +
                             (this.columnTooltip == 'normal' && col.tooltip ? 'title="'+ col.tooltip +'" ' : '') +
                        '    onmouseover = "wd[\''+ this.name +'\'].columnTooltipShow(\''+ i +'\', event);"'+
                        '    onmouseout  = "wd[\''+ this.name +'\'].columnTooltipHide(\''+ i +'\', event);"'+
                        '    oncontextmenu = "wd[\''+ this.name +'\'].contextMenu(null, '+ i +', event);"'+
                        '    onclick="wd[\''+ this.name +'\'].columnClick(\''+ col.field +'\', event);">'+
                             (col.resizable !== false ? '<div class="wd-resizer" name="'+ i +'"></div>' : '') +
                        '    <div class="wd-col-header '+ (sortStyle ? 'wd-col-sorted' : '') +' '+ (selected ? 'wd-col-selected' : '') +'">'+
                        '        <div class="'+ sortStyle +'"></div>'+
                                (!col.caption ? '&#160;' : col.caption) +
                        '    </div>'+
                        '</td>';
        ...
        getRecordsHTML: function () {
            ...
            html1 += '<tr id="grid_'+ this.name + '_frec_bottom" line="bottom" style="height: '+ ((buffered - limit) * this.recordHeight) +'px">'+
                    ...
                    '    <td colspan="2000" class="wd-load-more"></td>'+
                    '</tr>'+
                    '</tbody></table>';
        ...
        scroll: function (event) {
            ...
            if (colStart != null) {
                ...
                if (colStart != obj.last.colStart || colEnd != obj.last.colEnd) {
                    ...
                    if (deltaStart < 5 && deltaEnd < 5) {
                        var $cfirst = $box.find('.wd-grid-columns #grid_'+ obj.name +'_column_start');
                        var $clast  = $box.find('.wd-grid-columns .wd-head-last');
                        var $rfirst = $box.find('#grid_'+ obj.name +'_records .wd-grid-data-spacer');
                        var $rlast  = $box.find('#grid_'+ obj.name +'_records .wd-grid-data-last');
                        var $sfirst = $box.find('#grid_'+ obj.name +'_summary .wd-grid-data-spacer');
                        var $slast  = $box.find('#grid_'+ obj.name +'_summary .wd-grid-data-last');
                        // remove on left
                        ...
                        if (colStart < obj.last.colStart) {
                            for (var i = obj.last.colStart - 1; i >= colStart; i--) {
                                ...
                                $rfirst.each(function (ind, el) {
                                    ...
                                    var td    = '<td class="wd-grid-data" col="'+ i +'" style="height: 0px"></td>'; // width column
            ...
            $('#grid_'+ this.name + '_footer .wd-footer-right').html(
                    ...
                    if (more.css('display') == 'none') {
                        more.show()
                            .on('click', function () {
                                ...
                                $(this).find('td').html('<div><div style="width: 20px; height: 20px;" class="wd-spinner"></div></div>');
                            });
                    }
        ...
        getRecordHTML: function (ind, lineNum, summary) {
            ...
            if (ind == -1) {
                rec_html1 += '<tr line="0">';
                rec_html2 += '<tr line="0">';
                if (this.show.lineNumbers)  rec_html1 += '<td class="wd-col-number" style="height: 0px;"></td>';
                if (this.show.selectColumn) rec_html1 += '<td class="wd-col-select" style="height: 0px;"></td>';
                if (this.show.expandColumn) rec_html1 += '<td class="wd-col-expand" style="height: 0px;"></td>';
                rec_html2 += '<td class="wd-grid-data wd-grid-data-spacer" col="start" style="height: 0px; width: 0px;"></td>';
                for (var i = 0; i < this.columns.length; i++) {
                    var col = this.columns[i];
                    tmph = '<td class="wd-grid-data" col="'+ i +'" style="height: 0px;"></td>';
                ...
                rec_html1 += '<td class="wd-grid-data-last" style="height: 0px"></td>';
                rec_html2 += '<td class="wd-grid-data-last" col="end" style="height: 0px"></td>';
                rec_html1 += '</tr>';
                rec_html2 += '</tr>';
            ...
            rec_html1 += '<tr id="grid_'+ this.name +'_frec_'+ record.recid +'" recid="'+ record.recid +'" line="'+ lineNum +'" index="'+ ind +'" '+
                ' class="'+ (lineNum % 2 == 0 ? 'wd-even' : 'wd-odd') +
                    (isRowSelected && this.selectType == 'row' ? ' wd-selected' : '') +
                    (record.wd && record.wd.editable === false ? ' wd-no-edit' : '') +
                    (record.wd && record.wd.expanded === true ? ' wd-expanded' : '') + '" ' +
                (summary !== true ?
                    (w2utils.isIOS ?
                        '    onclick  = "wd[\''+ this.name +'\'].dblClick($(this).attr(\'recid\'), event);"'
                        :
                        '    onclick  = "wd[\''+ this.name +'\'].click($(this).attr(\'recid\'), event);"'+
                        '    oncontextmenu = "wd[\''+ this.name +'\'].contextMenu($(this).attr(\'recid\'), null, event);"'
                     )
                    : ''
                ) +
                (this.selectType == 'row' ?
                    ' onmouseover="$(\'#grid_'+ this.name +'_rec_\'+ w2utils.escapeId($(this).attr(\'recid\'))).addClass(\'wd-record-hover\')"'+
                    ' onmouseout ="$(\'#grid_'+ this.name +'_rec_\'+ w2utils.escapeId($(this).attr(\'recid\'))).removeClass(\'wd-record-hover\')"'
                    :
                    '') +
            ...
            rec_html2 += '<tr id="grid_'+ this.name +'_rec_'+ record.recid +'" recid="'+ record.recid +'" line="'+ lineNum +'" index="'+ ind +'" '+
                ...
                (summary !== true ?
                    (w2utils.isIOS ?
                        '    onclick  = "var obj = wd[\''+ this.name +'\']; obj.dblClick($(this).attr(\'recid\'), event);"'
                        :
                        '    onclick  = "var obj = wd[\''+ this.name +'\']; obj.click($(this).attr(\'recid\'), event);"'+
                        '    oncontextmenu = "var obj = wd[\''+ this.name +'\']; obj.contextMenu($(this).attr(\'recid\'), null, event);"'
                     )
                    : ''
                ) +
                (this.selectType == 'row' ?
                    ' onmouseover="$(\'#grid_'+ this.name +'_frec_\' + w2utils.escapeId($(this).attr(\'recid\'))).addClass(\'wd-record-hover\')"'+
                    ' onmouseout ="$(\'#grid_'+ this.name +'_frec_\' + w2utils.escapeId($(this).attr(\'recid\'))).removeClass(\'wd-record-hover\')"'
                    :
                    '') +
                ' style="height: '+ this.recordHeight +'px; '+ (!isRowSelected && typeof record['style'] == 'string' ? record['style'] : '') +'" '+
                    ( typeof record['style'] == 'string' ? 'custom_style="'+ record['style'] +'"' : '') +
            ...
            if (this.show.lineNumbers) {
                rec_html1 += '<td id="grid_'+ this.name +'_cell_'+ ind +'_number' + (summary ? '_s' : '') + '" '+
                            '   class="wd-col-number '+ (isRowSelected  ? ' wd-row-selected' : '') +'"'+
            ...
            if (this.show.selectColumn) {
                rec_html1 +=
                        '<td id="grid_'+ this.name +'_cell_'+ ind +'_select' + (summary ? '_s' : '') + '" class="wd-grid-data wd-col-select" '+
                        '        onclick="if (event.stopPropagation) event.stopPropagation(); else event.cancelBubble = true;">'+
                            (summary !== true ?
                            '    <div>'+
                            '        <input class="wd-grid-select-check" type="checkbox" tabindex="-1" '+ (isRowSelected ? 'checked="checked"' : '') +
                            '           onmousedown="if (event.stopPropagation) event.stopPropagation();"'+
                            '           onclick="var grid = wd[\''+ this.name +'\'];'+
            ...
            if (this.show.expandColumn) {
                var tmp_img = '';
                if (record.wd && record.wd.expanded === true)  tmp_img = '-'; else tmp_img = '+';
                if (record.wd && record.wd.expanded == 'none') tmp_img = '';
                if (record.wd && record.wd.expanded == 'spinner') tmp_img = '<div class="wd-spinner" style="width: 16px; margin: -2px 2px;"></div>';
                rec_html1 +=
                        '<td id="grid_'+ this.name +'_cell_'+ ind +'_expand' + (summary ? '_s' : '') + '" class="wd-grid-data wd-col-expand">'+
                            (summary !== true ?
                            '    <div ondblclick="if (event.stopPropagation) event.stopPropagation(); else event.cancelBubble = true;" '+
                            '            onclick="wd[\''+ this.name +'\'].toggle($(this).parents(\'tr\').attr(\'recid\')); '+
                            '                if (event.stopPropagation) event.stopPropagation(); else event.cancelBubble = true;">'+
                            '        '+ tmp_img +' </div>'
                            :
                            '' ) +
            ...
            rec_html2 += '<td class="wd-grid-data-spacer" col="start" style="border-right: 0"></td>';
            var col_ind   = 0;
            var col_skip  = 0;
            while (true) {
                var col_span  = 1;
                ...
                if (col_skip > 0) {
                    ...
                    record.wd.colspan[this.columns[col_ind].field] = 0; // need it for other methods
                    col_skip--;
                    continue;
                }
                // column virtual scroll
                ...
                if (record.wd) {
                    if (typeof record.wd.colspan == 'object') {
                        var span = parseInt(record.wd.colspan[col.field]) || null;
                        if (span > 1) {
            ...
            rec_html1 += '<td class="wd-grid-data-last"></td>';
            rec_html2 += '<td class="wd-grid-data-last" col="end"></td>';
            rec_html1 += '</tr>';
            rec_html2 += '</tr>';
            return [rec_html1, rec_html2];
        },
        ...
        getCellHTML: function (ind, col_ind, summary, col_span) {
            ...
            var isChanged     = !summary && record.wd && record.wd.changes && record.wd.changes[col.field] != null;
            var addStyle      = '';
            var sel           = this.last.selection;
            var isRowSelected = false;
            var infoBubble    = '';
            ...
            // expand icon
            if (col_ind == 0 && record.wd && Array.isArray(record.wd.children)) {
                var level  = 0;
                var subrec = this.get(record.wd.parent_recid, true);
                while (true) {
                    if (subrec != null) {
                        level++
                        var tmp = this.records[subrec].wd;
                ...
                for (var i = 0; i < level; i++) {
                    infoBubble += '<span class="wd-show-children wd-icon-empty"></span>';
                }
                infoBubble += '<span class="wd-show-children '+
                        (record.wd.children.length > 0
                            ? (record.wd.expanded ? 'wd-icon-collapse' : 'wd-icon-expand')
                            : 'wd-icon-empty'
                        ) +'" '+
                    ' onclick="event.stopPropagation(); wd[\''+ this.name + '\'].toggle($(this).parents(\'tr\').attr(\'recid\'))"></span>';
            }
            // info bubble
            if (col.info === true) col.info = {};
            if (col.info != null) {
                if (!col.info.icon) col.info.icon = 'wd-icon-info';
                infoBubble += '<span class="wd-info '+ col.info.icon +'" style="'+ (col.info.style || '') + '" '+
                    ' onclick="event.stopPropagation(); wd[\''+ this.name + '\'].showBubble('+ ind +', '+ col_ind +')"></span>';
            }
            // various renderers
            if (col.render != null) {
            ...
            if (record.wd && typeof record.wd.style == 'object') {
                if (typeof record.wd.style[col_ind] == 'string') addStyle += record.wd.style[col_ind] + ';';
                if (typeof record.wd.style[col.field] == 'string') addStyle += record.wd.style[col.field] + ';';
            }
            var isCellSelected = false;
            if (isRowSelected && $.inArray(col_ind, sel.columns[ind]) != -1) isCellSelected = true;
            // data
            data =  '<td class="wd-grid-data'+ (isCellSelected ? ' wd-selected' : '') +
                        (isChanged ? ' wd-changed' : '') +
                        '" '+
        ...
        showBubble: function (ind, col_ind) {
            ...
            var rec  = this.records[ind];
            var el   = $(this.box).find('#grid_'+ this.name +'_data_'+ ind +'_'+ col_ind + ' .wd-info');
        ...
        getCellValue: function (ind, col_ind, summary) {
            ...
            if (record.wd && record.wd.changes && record.wd.changes[col.field] != null) {
                data = record.wd.changes[col.field];
            }
        ...
        getFooterHTML: function () {
            return '<div>'+
                '    <div class="wd-footer-left"></div>'+
                '    <div class="wd-footer-right"></div>'+
                '    <div class="wd-footer-center"></div>'+
                '</div>';
        },

        status: function (msg) {
            if (msg != null) {
                $('#grid_'+ this.name +'_footer').find('.wd-footer-left').html(msg);
            } else {
                // show number of selected
                var msgLeft = '';
                var sel = this.getSelection();
                ...
                $('#grid_'+ this.name +'_footer .wd-footer-left').html(msgLeft);
                // toolbar
                if (sel.length == 1) this.toolbar.enable('wd-edit'); else this.toolbar.disable('wd-edit');
                if (sel.length >= 1) this.toolbar.enable('wd-delete'); else this.toolbar.disable('wd-delete');
            }
        ...
        stateSave: function (returnOnly) {
            ...
            if (returnOnly !== true) {
                ...
                if (eventData.isCancelled === true) { if (typeof callBack == 'function') callBack({ status: 'error', message: 'Request aborted.' }); return; }
                try {
                    var savedState = $.parseJSON(localStorage.wd || '{}');
                    if (!savedState) savedState = {};
                    if (!savedState.states) savedState.states = {};
                    savedState.states[this.name] = state;
                    localStorage.wd = JSON.stringify(savedState);
                } catch (e) {
        ...
        stateRestore: function (newState) {
            ...
            if (!newState) {
                ...
                try {
                    ...
                    var tmp = $.parseJSON(localStorage.wd || '{}');
                    if (!tmp) tmp = {};
        ...
        stateReset: function () {
            ...
            if (localStorage) {
                try {
                    ...
                    localStorage.wd = JSON.stringify(tmp);
                } catch (e) {
                    delete localStorage.wd;
                    return null;
                }
...
(function ($) {
    var w2layout = function (options) {
        ...
        this.name    = null;        // unique name for wd
        this.panels  = [];
        this.tmp     = {};

        this.padding = 6;        // panel padding
    ...
    w2layout.prototype = {
        ...
        content: function (panel, data, transition) {
            ...
            if (data == null) {
                ...
                var current  = $(pname + '> .wd-panel-content');
                var panelTop = 0;
                if (current.length > 0) {
                    $(pname).scrollTop(0);
                ...
                if (p.content === '') {
                    ...
                    if (!p.hidden) {
                        if (transition != null && transition !== '') {
                            // apply transition
                            var div1 = $(pname + '> .wd-panel-content');
                            div1.after('<div class="wd-panel-content new-panel" style="'+ div1[0].style.cssText +'"></div>');
                            var div2 = $(pname + '> .wd-panel-content.new-panel');
        ...
        sizeTo: function (panel, size, instant) {
            ...
            // clean
            setTimeout(function () {
                $(obj.box).find(' > div > .wd-panel').css(w2utils.cssPrefix('transition', '0s'));
                obj.resize();
            }, 500);
            return true;
        },
        ...
        show: function (panel, immediate) {
            ...
            if (immediate === true) {
                ...
                $(obj.box).find(' > div > .wd-panel').css(w2utils.cssPrefix('transition', '.2s'));
        ...
        el: function (panel) {
            var el = $('#layout_'+ this.name +'_panel_'+ panel +'> .wd-panel-content');
        ...
        hideToolbar: function (panel) {
            ...
            $('#layout_'+ this.name +'_panel_'+ panel +'> .wd-panel-toolbar').hide();
            this.resize();
        },
        ...
        showToolbar: function (panel) {
            ...
            if (!pan) return;
            pan.show.toolbar = true;
            $('#layout_'+ this.name +'_panel_'+ panel +'> .wd-panel-toolbar').show();
            this.resize();
        },
        ...
        assignToolbar: function (panel, toolbar) {
            if (typeof toolbar == 'string' && wd[toolbar] != null) toolbar = wd[toolbar];
            var pan = this.get(panel);
            pan.toolbar = toolbar;
            var tmp = $(this.box).find(panel +'> .wd-panel-toolbar');
            if (pan.toolbar != null) {
        ...
        hideTabs: function (panel) {
            ...
            if (!pan) return;
            pan.show.tabs = false;
            $('#layout_'+ this.name +'_panel_'+ panel +'> .wd-panel-tabs').hide();
            this.resize();
        },
        ...
        showTabs: function (panel) {
            ...
            if (!pan) return;
            pan.show.tabs = true;
            $('#layout_'+ this.name +'_panel_'+ panel +'> .wd-panel-tabs').show();
            this.resize();
        },
        ...
        render: function (box) {
            ...
            for (var p1 = 0; p1 < w2panels.length; p1++) {
                ...
                var html =  '<div id="layout_'+ obj.name + '_panel_'+ w2panels[p1] +'" class="wd-panel">'+
                            '    <div class="wd-panel-title"></div>'+
                            '    <div class="wd-panel-tabs"></div>'+
                            '    <div class="wd-panel-toolbar"></div>'+
                            '    <div class="wd-panel-content"></div>'+
                            '</div>'+
                            '<div id="layout_'+ obj.name + '_resizer_'+ w2panels[p1] +'" class="wd-resizer"></div>';
            ...
            function resizeStart(type, evnt) {
                ...
                // lock all panels
                for (var p1 = 0; p1 < w2panels.length; p1++) {
                    var $tmp = $(obj.el(w2panels[p1])).parent().find('.wd-lock');
                    if ($tmp.length > 0) {
        ...
        refresh: function (panel) {
            ...
            if (typeof panel == 'string') {
                ...
                if (typeof p.content == 'object' && typeof p.content.render === 'function') {
                    p.content.box = $(pname +'> .wd-panel-content')[0];
                    setTimeout(function () {
                        // need to remove unnecessary classes
                        if ($(pname +'> .wd-panel-content').length > 0) {
                            $(pname +'> .wd-panel-content')
                                .removeClass()
                                .removeAttr('name')
                                .addClass('wd-panel-content')
        ...
        resize: function () {
            ...
            if (ptop != null && ptop.hidden !== true) {
                ...
                if (ptop.resizable) {
                    ...
                    $('#layout_'+ this.name +'_resizer_top').show().css({
                        ...
                        if (eventData.isCancelled === true) return;
                        // default action
                        wd[obj.name].tmp.events.resizeStart('top', event);
                        // event after
                        obj.trigger($.extend(eventData, { phase: 'after' }));
                        return false;
                    });
            ...
            if (pleft != null && pleft.hidden !== true) {
                ...
                if (pleft.resizable) {
                    ...
                    $('#layout_'+ this.name +'_resizer_left').show().css({
                        ...
                        if (eventData.isCancelled === true) return;
                        // default action
                        wd[obj.name].tmp.events.resizeStart('left', event);
                        // event after
                        obj.trigger($.extend(eventData, { phase: 'after' }));
                        return false;
                    });
            ...
            if (pright != null && pright.hidden !== true) {
                ...
                if (pright.resizable) {
                    ...
                    $('#layout_'+ this.name +'_resizer_right').show().css({
                        ...
                        if (eventData.isCancelled === true) return;
                        // default action
                        wd[obj.name].tmp.events.resizeStart('right', event);
                        // event after
                        obj.trigger($.extend(eventData, { phase: 'after' }));
                        return false;
                    });
            ...
            if (pbottom != null && pbottom.hidden !== true) {
                ...
                if (pbottom.resizable) {
                    ...
                    $('#layout_'+ this.name +'_resizer_bottom').show().css({
                        ...
                        if (eventData.isCancelled === true) return;
                        // default action
                        wd[obj.name].tmp.events.resizeStart('bottom', event);
                        // event after
                        obj.trigger($.extend(eventData, { phase: 'after' }));
                        return false;
                    });
            ...
            if (pprev != null && pprev.hidden !== true) {
                ...
                if (pprev.resizable) {
                    ...
                    $('#layout_'+ this.name +'_resizer_preview').show().css({
                        ...
                        if (eventData.isCancelled === true) return;
                        // default action
                        wd[obj.name].tmp.events.resizeStart('preview', event);
                        // event after
                        obj.trigger($.extend(eventData, { phase: 'after' }));
                        return false;
                    });
            ...
            for (var p1 = 0; p1 < w2panels.length; p1++) {
                ...
                var tmp2 = '#layout_'+ this.name +'_panel_'+ w2panels[p1] +' > .wd-panel-';
                var tabHeight = 0;
                if (pan) {
                    if (pan.title) {
                    ...
                    if (pan.show.tabs) {
                        if (pan.tabs != null && wd[this.name +'_'+ w2panels[p1] +'_tabs']) wd[this.name +'_'+ w2panels[p1] +'_tabs'].resize();
                        tabHeight += w2utils.getSize($(tmp2 + 'tabs').css({ top: tabHeight + 'px', display: 'block' }), 'height');
                    }
                    if (pan.show.toolbar) {
                        if (pan.toolbar != null && wd[this.name +'_'+ w2panels[p1] +'_toolbar']) wd[this.name +'_'+ w2panels[p1] +'_toolbar'].resize();
            ...
            this._resize_timer = setTimeout(function () {
                for (var e in wd) {
                    if (typeof wd[e].resize == 'function') {
                        // sent to all none-layouts
                        if (wd[e].panels == null) wd[e].resize();
                        // only send to nested layouts
                        var parent = $(wd[e].box).parents('.wd-layout');
                        if (parent.length > 0 && parent.attr('name') == obj.name) wd[e].resize();
                    }
                }
            }, 100);
            this.trigger($.extend(eventData, { phase: 'after' }));
        ...
        destroy: function () {
            ...
            if (eventData.isCancelled === true) return;
            if (wd[this.name] == null) return false;
            // clean up
...
(function ($) {
    ...
    $.fn.w2popup = function(method, options) {
        ...
        if ($(this).length > 0 && method == 'open') {
            if ($(this).find('div[rel=title], div[rel=body], div[rel=buttons]').length > 0) {
                ...
                if ($('#wd-popup').length > 0) {
                    var tmp  = $('#wd-popup').data('options');
                    w2popup._prev = {
                        /*
    ...
    w2popup = {
        ...
        open: function (options) {
            ...
            var old_options = $('#wd-popup').data('options');
            ...
            setTimeout(function () { $('#wd-popup').data('options', options); }, 100);
            ...
            if ($('#wd-popup').length == 0) {
                ...
                if (options.showClose) {
                    btn += '<div class="wd-msg-button wd-msg-close" onmousedown="event.stopPropagation()" onclick="w2popup.close()">Close</div>';
                }
                if (options.showMax) {
                    btn += '<div class="wd-msg-button wd-msg-max" onmousedown="event.stopPropagation()" onclick="w2popup.toggle()">Max</div>';
                }
                // first insert just body
                var msg = '<div id="wd-popup" class="wd-popup" style="opacity: 0; left: '+ left +'px; top: '+ top +'px;'+
                ...
                // then content
                var msg = '<div class="wd-msg-title" style="'+ (!options.title ? 'display: none' : '') +'">' + btn + '</div>'+
                          '<div class="wd-box" style="'+ (!options.title ? 'top: 0px !important;' : '') + 
                                    (!options.buttons ? 'bottom: 0px !important;' : '') + '">'+
                          '    <div class="wd-msg-body' + (!options.title != '' ? ' wd-msg-no-title' : '') + 
                                    (!options.buttons ? ' wd-msg-no-buttons' : '') + '" style="' + options.style + '">' + 
                          '    </div>'+
                          '</div>'+
                          '<div class="wd-msg-buttons" style="'+ (!options.buttons ? 'display: none' : '') +'"></div>'+
                          '<input class="wd-popup-hidden" style="position: absolute; top: -100px"/>'; // this is needed to keep focus in popup
                $('#wd-popup').html(msg);

                if (options.title) $('#wd-popup .wd-msg-title').append(options.title);
                if (options.buttons) $('#wd-popup .wd-msg-buttons').append(options.buttons);
                if (options.body) $('#wd-popup .wd-msg-body').append(options.body);
                ...
                // clean transform
                setTimeout(function () {
                    $('#wd-popup').css(w2utils.cssPrefix('transform', ''));
                    // event after
                    w2popup.status = 'open';
                    setTimeout(function () {
                        obj.trigger($.extend(eventData, { phase: 'after' }));
                ...
                // show new items
                var cloned = $('#wd-popup .wd-box').clone();
                cloned.removeClass('wd-box').addClass('wd-box-temp').find('.wd-msg-body').empty().append(options.body);
                // parse rel=*
                ...
                $('#wd-popup .wd-box').after(cloned);

                if (options.buttons) {
                    $('#wd-popup .wd-msg-buttons').show().html('').append(options.buttons);
                    $('#wd-popup .wd-msg-body').removeClass('wd-msg-no-buttons');
                    $('#wd-popup .wd-box, #wd-popup .wd-box-temp').css('bottom', '');
                } else {
                    $('#wd-popup .wd-msg-buttons').hide().html('');
                    $('#wd-popup .wd-msg-body').addClass('wd-msg-no-buttons');
                    $('#wd-popup .wd-box, #wd-popup .wd-box-temp').css('bottom', '0px');
                }
                if (options.title) {
                    $('#wd-popup .wd-msg-title')
                        .show()
                        .html((options.showClose ? '<div class="wd-msg-button wd-msg-close" onmousedown="event.stopPropagation()" onclick="w2popup.close()">Close</div>' : '') +
                              (options.showMax ? '<div class="wd-msg-button wd-msg-max" onmousedown="event.stopPropagation()" onclick="w2popup.toggle()">Max</div>' : ''))
                        .append(options.title);
                    $('#wd-popup .wd-msg-body').removeClass('wd-msg-no-title');
                    $('#wd-popup .wd-box, #wd-popup .wd-box-temp').css('top', '');
                } else {
                    $('#wd-popup .wd-msg-title').hide().html('');
                    $('#wd-popup .wd-msg-body').addClass('wd-msg-no-title');
                    $('#wd-popup .wd-box, #wd-popup .wd-box-temp').css('top', '0px');
                }
                // transition
                var div_old = $('#wd-popup .wd-box')[0];
                var div_new = $('#wd-popup .wd-box-temp')[0];
                w2utils.transition(div_old, div_new, options.transition, function () {
                    // clean up
                    obj._restoreTemplate();
                    $(div_old).remove();
                    $(div_new).removeClass('wd-box-temp').addClass('wd-box');
                    var $body = $(div_new).find('.wd-msg-body');
                    if ($body.length == 1) $body[0].style.cssText = options.style;
                    // remove max state
                    $('#wd-popup').data('prev-size', null);
            ...
            $('#wd-popup .wd-msg-title').on('mousedown', function (event) { 
            ...
            function mvStart(evnt) {
                ...
                tmp.resizing = true;
                tmp.isLocked = $('#wd-popup > .wd-lock').length == 1 ? true : false;
                tmp.x = evnt.screenX;
                tmp.y = evnt.screenY;
                tmp.pos_x = $('#wd-popup').position().left;
                tmp.pos_y = $('#wd-popup').position().top;
            ...
            function mvMove(evnt) {
                ...
                $('#wd-popup').css(w2utils.cssPrefix({
                    'transition': 'none',
        ...
        keydown: function (event) {
            var options = $('#wd-popup').data('options');
            ...
            switch (event.keyCode) {
                case 27:
                    event.preventDefault();
                    if ($('#wd-popup .wd-popup-message').length > 0) w2popup.message(); else w2popup.close();
                    break;
            }
            // event after
        ...
        close: function (options) {
            var obj = this;
            var options = $.extend({}, $('#wd-popup').data('options'), options);
            if ($('#wd-popup').length == 0 || this.status == 'closed') return;
        ...
        max: function () {
            ...
            options.prevSize = $('#wd-popup').css('width') + ':' + $('#wd-popup').css('height');
            // do resize
            w2popup.resize(10000, 10000, function () {
                ...
                $('#wd-popup .wd-grid, #wd-popup .wd-form, #wd-popup .wd-layout').each(function () {
                    var name = $(this).attr('name');
                    if (wd[name] && wd[name].resize) wd[name].resize();
                })
            });
        },
        ...
        clear: function() {
            $('#wd-popup .wd-msg-title').html('');
            $('#wd-popup .wd-msg-body').html('');
            $('#wd-popup .wd-msg-buttons').html('');
        },
        ...
        load: function (options) {
            ...
            // load url
            var html = $('#wd-popup').data(url);
            if (html != null) {
                popup(html, selector);
            } else {
                $.get(url, function (data, status, obj) { // should always be $.get as it is template
                    popup(obj.responseText, selector);
                    $('#wd-popup').data(url, obj.responseText); // remember for possible future purposes
                });
            }
            function popup(html, selector) {
                delete options.url;
                $('body').append('<div id="wd-tmp" style="display: none">' + html + '</div>');
                if (selector != null && $('#wd-tmp #'+selector).length > 0) {
                    $('#wd-tmp #' + selector).w2popup(options);
                } else {
                    $('#wd-tmp > div').w2popup(options);
                }
                // link styles
                if ($('#wd-tmp > style').length > 0) {
                    var style = $('<div>').append($('#wd-tmp > style').clone()).html();
                    if ($('#wd-popup #div-style').length == 0) {
                        $('#wd-popup').append('<div id="div-style" style="position: absolute; left: -100; width: 1px"></div>');
                    }
                    $('#wd-popup #div-style').html(style);
                }
                $('#wd-tmp').remove();
            }
        },
        ...
        message: function (options) {
            ...
            var pWidth   = parseInt($('#wd-popup').width());
            var pHeight  = parseInt($('#wd-popup').height());
            options.originalWidth  = options.width;
            options.originalHeight = options.height;
            if (parseInt(options.width) < 10)  options.width  = 10;
            if (parseInt(options.height) < 10) options.height = 10;
            if (options.hideOnClick == null) options.hideOnClick = false;
            var poptions    = $('#wd-popup').data('options') || {};
            var titleHeight = parseInt($('#wd-popup > .wd-msg-title').css('height'));
            ...
            var head     = $('#wd-popup .wd-msg-title');
            var msgCount = $('#wd-popup .wd-popup-message').length;
            // remove message
            if ($.trim(options.html) == '' && $.trim(options.body) == '' && $.trim(options.buttons) == '') {
                var $msg = $('#wd-popup #wd-message'+ (msgCount-1));
                ...
                if (msgCount == 1) {
                    ...
                    $('#wd-popup #wd-message'+ (msgCount-2)).css('z-index', 1500);
                }
                setTimeout(function () {
                ...
                if ($.trim(options.body) != '' || $.trim(options.buttons) != '') {
                    options.html = '<div class="wd-popup-message-body">'+ options.body +'</div>'+
                        '<div class="wd-popup-message-buttons">'+ options.buttons +'</div>';
                }
                // hide previous messages
                $('#wd-popup .wd-popup-message').css('z-index', 1390);
                head.css('z-index', 1501);
                // add message
                $('#wd-popup .wd-box')
                    .before('<div id="wd-message' + msgCount + '" class="wd-popup-message" style="display: none; z-index: 1500; ' +
                            ...
                            '</div>');
                $('#wd-popup #wd-message'+ msgCount).data('options', options).data('prev_focus', $(':focus'));
                var display = $('#wd-popup #wd-message'+ msgCount).css('display');
                $('#wd-popup #wd-message'+ msgCount).css(w2utils.cssPrefix({
                    'transform': (display == 'none' ? 'translateY(-' + options.height + 'px)' : 'translateY(0px)')
                }));
                if (display == 'none') {
                    $('#wd-popup #wd-message'+ msgCount).show().html(options.html);
                    ...
                    setTimeout(function() {
                        ...
                        $('#wd-popup #wd-message'+ msgCount).css(w2utils.cssPrefix({ 'transition': '0s' }));
        ...
        focus: function () {
            ...
            // in message or popup
            var cnt = $('#wd-popup .wd-popup-message').length - 1;
            var msg = $('#wd-popup #wd-message' + cnt);
            if (msg.length > 0) {
                var btn =$(msg[msg.length - 1]).find('button');
                if (btn.length > 0) btn[0].focus();
                tmp = msg;
            } else if (pop.length > 0) {
                var btn = pop.find('.wd-msg-buttons button');
                if (btn.length > 0) btn[0].focus();
                tmp = pop;
            }
            ...
            $(tmp).find(sel)
                .on('blur.keep-focus', function (event) {
                    setTimeout(function () {
                        var focus = $(':focus');
                        if ((focus.length > 0 && !$(tmp).find(sel).is(focus)) || focus.hasClass('wd-popup-hidden')) {
                            var el = $(tmp).find(sel);
        ...
        lock: function (msg, showSpinner) {
            var args = Array.prototype.slice.call(arguments, 0);
            args.unshift($('#wd-popup'));
            w2utils.lock.apply(window, args);
        },

        unlock: function (speed) {
            w2utils.unlock($('#wd-popup'), speed);
        },

        // --- INTERNAL FUNCTIONS

        lockScreen: function (options) {
            if ($('#wd-lock').length > 0) return false;
            if (options == null) options = $('#wd-popup').data('options');
            if (options == null) options = {};
            options = $.extend({}, w2popup.defaults, options);
            // show element
            $('body').append('<div id="wd-lock" ' +
            ...
            if (options.modal == true) {
                /*
                $('#wd-lock').on('mousedown', function () {
                    $('#wd-lock')
                        .css('opacity', '0.6')
                        .css(w2utils.cssPrefix('transition', '.1s'));
                });
                $('#wd-lock').on('mouseup', function () {
                    setTimeout(function () {
                        $('#wd-lock')
                            .css('opacity', options.opacity)
                            .css(w2utils.cssPrefix('transition', '.1s'));
...
*/
                ...
                $('#wd-lock').on('mousedown', function () { w2popup.close(); });
            }
            return true;
        },
        ...
        resizeMessages: function () {
            ...
            $('#wd-popup .wd-popup-message').each(function () {
                ...
                var titleHeight = parseInt($popup.find('> .wd-msg-title').css('height'));
...
var w2alert = function (msg, title, callBack) {
    ...
    if ($('#wd-popup').length > 0 && w2popup.status != 'closing') {
        w2popup.message({
            width   : 400,
            height  : 170,
            body    : '<div class="wd-centered wd-alert-msg" style="font-size: 13px;">' + msg + '</div>',
            buttons : '<button onclick="w2popup.message();" class="wd-popup-btn wd-btn">' + w2utils.lang('Ok') + '</button>',            
            onOpen: function () {
                $('#wd-popup .wd-popup-message .wd-popup-btn').focus();
            },
            onClose: function () {
                if (typeof callBack == 'function') callBack();
            }
        ...
        w2popup.open({
            ...
            title     : title,
            body      : '<div class="wd-centered wd-alert-msg" style="font-size: 13px;">' + msg + '</div>',
            buttons   : '<button onclick="w2popup.close();" class="wd-popup-btn wd-btn">' + w2utils.lang('Ok') + '</button>',
            onOpen: function (event) {
                // do not use onComplete as it is slower
                setTimeout(function () { $('#wd-popup .wd-popup-btn').focus(); }, 1);
            },
            onKeydown: function (event) {
                $('#wd-popup .wd-popup-btn').focus().addClass('clicked'); 
            },
            onClose: function () {
                if (typeof callBack == 'function') callBack();
            }
...
var w2confirm = function (msg, title, callBack) {
    ...
    var defaults = {
        ...
        width       : ($('#wd-popup').length > 0 ? 400 : 450),
        height      : ($('#wd-popup').length > 0 ? 170 : 220),
        yes_text    : 'Yes',
        yes_class   : '',
        yes_style   : '',
        yes_callBack: null,
    ...
    if ($('#wd-popup').length > 0 && w2popup.status != 'closing' && w2popup.get()) {
        ...
        if (options.height > (w2popup.get().height - 50)) options.height = w2popup.get().height - 50;
          w2popup.message({
            ...
            body    : '<div class="wd-centered wd-confirm-msg" style="font-size: 13px;">' + options.msg + '</div>',
            buttons : '<button id="Yes" class="wd-popup-btn wd-btn '+ options.yes_class +'" style="'+ options.yes_style +'">' + w2utils.lang(options.yes_text) + '</button>' +
                      '<button id="No" class="wd-popup-btn wd-btn '+ options.no_class +'" style="'+ options.no_style +'">' + w2utils.lang(options.no_text) + '</button>',
            onOpen: function () {
                $('#wd-popup .wd-popup-message .wd-btn').on('click.w2confirm', function (event) {
            ...
            onClose: function () {
                ...
                $('#wd-popup .wd-popup-message .wd-btn').off('click.w2confirm');   
        ...
        w2popup.open({
            ...
            onOpen: function (event) {
                // do not use onComplete as it is slower
                setTimeout(function () {
                    $('#wd-popup .wd-popup-btn').on('click', function (event) {
                        w2popup.close();
                    ...
                    $('#wd-popup .wd-popup-btn#Yes').focus();
                }, 1);
            },
            onKeydown: function (event) {
                // if there are no messages
                if ($('#wd-popup .wd-popup-message').length == 0) {
                    switch (event.originalEvent.keyCode) {
                        case 13: // enter
                            $('#wd-popup .wd-popup-btn#Yes').focus().addClass('clicked'); // no need fo click as enter will do click
                            w2popup.close();
                            break;
                        case 27: // esc
                            $('#wd-popup .wd-popup-btn#No').focus().click();
                            w2popup.close();
                            break;
                    }
                }
...
(function ($) {
    ...
    w2tabs.prototype = {
        ...
        refresh: function (id) {
            var time = (new Date()).getTime();
            if (this.flow == 'up') $(this.box).addClass('wd-tabs-up'); else $(this.box).removeClass('wd-tabs-up');
            // event before
            ...
            if (id == null) {
                ...
                if (tab.closable && !tab.disabled) {
                    closable = '<div class="wd-tab-close" '+
                               '    onmouseover = "wd[\''+ this.name +'\'].tooltipShow(\''+ tab.id +'\', event);"'+
                               '    onmouseout  = "wd[\''+ this.name +'\'].tooltipHide(\''+ tab.id +'\', event);"'+
                               '    onclick="wd[\''+ this.name +'\'].animateClose(\''+ tab.id +'\', event);">'+
                               '</div>';
                }
                var tabHTML = closable +
                    '    <div class="wd-tab'+ (this.active === tab.id ? ' active' : '') + (tab.closable ? ' closable' : '') 
                                + (tab['class'] ? ' ' + tab['class'] : '') +'" '+
                    '        title="'+ (this.tooltip == 'normal' && tab.tooltip != null ? tab.tooltip : '') +'" style="'+ tab.style +'" '+ 
                    '        onmouseover = "' + (!tab.disabled ? "wd['"+ this.name +"'].tooltipShow('"+ tab.id +"', event);" : "") + '"'+
                    '        onmouseout  = "' + (!tab.disabled ? "wd['"+ this.name +"'].tooltipHide('"+ tab.id +"', event);" : "") + '"'+                    
                    '        onclick="wd[\''+ this.name +'\'].click(\''+ tab.id +'\', event);">' + tab.text + '</div>';
                if (jq_el.length === 0) {
        ...
        click: function (id, event) {
            ...
            if (eventData.isCancelled === true) return;
            // default action
            $(this.box).find('#tabs_'+ this.name +'_tab_'+ w2utils.escapeId(this.active) +' .wd-tab').removeClass('active');
            this.active = tab.id;
            // route processing
        ...
        animateInsert: function(id, tab) {
            ...
            // measure width
            if (tab.text == null && tab.caption != null) tab.text = tab.caption;
            var tmp = '<div id="_tmp_tabs" class="wd-reset wd-tabs" style="position: absolute; top: -1000px;">'+
                '<table cellspacing="0" cellpadding="1" width="100%"><tbody><tr>'+
                '<td id="_tmp_simple_tab" style="" valign="middle">'+
                    (tab.closable ? '<div class="wd-tab-close"></div>' : '') +
                '    <div class="wd-tab '+ (this.active === tab.id ? 'active' : '') +'">'+ tab.text +'</div>'+
                '</td></tr></tbody></table>'+
                '</div>';
            $('body').append(tmp);
    ...
    w2toolbar.prototype = {
        ...
        click: function (id, event) {
            ...
            if (it && !it.disabled) {
                ...
                var btn = '#tb_'+ this.name +'_item_'+ w2utils.escapeId(it.id) +' table.wd-button';
        ...
        scroll: function (direction) {
            var box = $(this.box);
            var scrollBox = box.children('.wd-toolbar-scroll-wrapper');
        ...
        render: function (box) {
            ...
            if (box != null) {
                if ($(this.box).find('> table #tb_'+ this.name + '_right').length > 0) {
                    $(this.box)
                        .removeAttr('name')
                        .removeClass('wd-reset wd-toolbar')
                        .html('');
                }
                this.box = box;
            }
            if (!this.box) return;
            // render all buttons
            var html = '<div class="wd-toolbar-scroll-left" onclick="var el=wd[\''+ this.name + '\']; if (el) el.scroll(\'left\');"></div>'+
                       '<div class="wd-toolbar-scroll-wrapper"><table cellspacing="0" cellpadding="0" width="100%"><tbody>'+
                       '<tr>';
            for (var i = 0; i < this.items.length; i++) {
                var it = this.items[i];
                if (it == null)  continue;
            ...
            html += '</tr>'+
                    '</tbody></table></div>'+
                    '<div class="wd-toolbar-scroll-right" onclick="var el=wd[\''+ this.name + '\']; if (el) el.scroll(\'right\');"></div>';
            $(this.box)
                .attr('name', this.name)
                .addClass('wd-reset wd-toolbar')
                .html(html);
            if ($(this.box).length > 0) $(this.box)[0].style.cssText += this.style;
            // event after
            this.trigger($.extend(eventData, { phase: 'after' }));
        ...
        refresh: function (id) {
            ...
            if (el.length == 0) {
                ...
                if (['menu', 'menu-radio', 'menu-check', 'drop', 'color', 'text-color'].indexOf(it.type) != -1 && it.checked == true) {
                    ...
                    if ($('#wd-overlay').length > 0) $('#wd-overlay')[0].hide();
                }
                // refresh
                el.html(html);
        ...
        getItemHTML: function (item) {
            ...
            if (item.img)  img = '<td><div class="wd-tb-image wd-icon '+ item.img +'"></div></td>';
            if (item.icon) img = '<td><div class="wd-tb-image"><span class="'+ item.icon +'"></span></div></td>';
            ...
            switch (item.type) {
                ...
                case 'drop':
                    html += '<table cellpadding="0" cellspacing="0" '+ (this.tooltip == 'normal' && item.tooltip != null ? 'title="'+ w2utils.lang(item.tooltip) +'"' : '') +
                            '       class="wd-button '+ (item.checked ? 'checked' : '') +'" '+
                            '       onclick     = "var el=wd[\''+ this.name + '\']; if (el) el.click(\''+ item.id +'\', event);" '+
                            '       onmouseover = "' + (!item.disabled ? "$(this).addClass('over'); wd['"+ this.name +"'].tooltipShow('"+ item.id +"', event);" : "") + '"'+
                            '       onmouseout  = "' + (!item.disabled ? "$(this).removeClass('over').removeClass('down'); wd['"+ this.name +"'].tooltipHide('"+ item.id +"', event);" : "") + '"'+
                            ...
                            '  <tr>' +
                                    img +
                                    (text !== ''
                                        ? '<td class="wd-tb-caption" nowrap="nowrap" style="'+ (item.style ? item.style : '') +'">'+ w2utils.lang(text) +'</td>'
                                        : ''
                                    ) +
                                    (item.count != null
                                        ? '<td class="wd-tb-count" nowrap="nowrap"><span>'+ item.count +'</span></td>'
                                        : ''
                                    ) +
                                    (((['menu', 'menu-radio', 'menu-check', 'drop', 'color', 'text-color'].indexOf(item.type) != -1) && item.arrow !== false) ?
                                        '<td class="wd-tb-down" nowrap="nowrap"><div></div></td>' : '') +
                            '  </tr></tbody></table>'+
                            '</td></tr></tbody></table>';
                    break;
...
(function ($) {
    var w2tree = function (options) {
        ...
        $.extend(true, this, w2obj.tree, options);
    };
    ...
    $.fn.w2tree = function(method) {
        if ($.isPlainObject(method)) {
            // check name parameter
            if (!w2utils.checkName(method, 'w2tree')) return;
            // extend items
    ...
    w2tree.prototype = {
        ...
        insert: function (parent, before, nodes) {
            ...
            for (var o = 0; o < nodes.length; o++) {
                ...
                tmp = $.extend({}, w2tree.prototype.node, node);
                tmp.tree = this;
                tmp.parent  = parent;
                nd = tmp.nodes || [];
        ...
        remove: function () { // multiple arguments
            ...
            for (var a = 0; a < arguments.length; a++) {
                ...
                if (tmp.parent.nodes[ind].selected)    tmp.tree.unselect(tmp.id);
        ...
        select: function (id) {
            ...
            $(this.box).find('#node_'+ w2utils.escapeId(id))
                .addClass('wd-selected')
                .find('.wd-icon').addClass('wd-icon-selected');
        ...
        unselect: function (id) {
            ...
            $(this.box).find('#node_'+ w2utils.escapeId(id))
                .removeClass('wd-selected')
                .find('.wd-icon').removeClass('wd-icon-selected');
        ...
        collapse: function (id) {
            ...
            $(this.box).find('#node_'+ w2utils.escapeId(id) +' .wd-node-dots:first-child').html('<div class="wd-expand">+</div>');
            nd.expanded = false;
            // event after
            this.trigger($.extend(eventData, { phase: 'after' }));
        ...
        click: function (id, event) {
            ...
            setTimeout(function () {
                ...
                if (eventData.isCancelled === true) {
                    // restore selection
                    newNode.removeClass('wd-selected').find('.wd-icon').removeClass('wd-icon-selected');
                    oldNode.addClass('wd-selected').find('.wd-icon').addClass('wd-icon-selected');
                    return;
                }
                // default action
                if (oldNode != null) oldNode.selected = false;
        ...
        focus: function (event) {
            ...
            // default behaviour
            this.hasFocus = true;
            $(this.box).find('.wd-selected').removeClass('wd-inactive');
            setTimeout(function () {
                var $input = $(obj.box).find('#tree_'+ obj.name + '_focus');
                if (!$input.is(':focus')) $input.focus();
            }, 10);
            // event after
            this.trigger($.extend(eventData, { phase: 'after' }));
        ...
        scrollIntoView: function (id) {
            ...
            var nd = this.get(id);
            if (nd == null) return;
            var body   = $(this.box).find('.wd-tree-div');
        ...
        render: function (box) {
            ...
            // default action
            if (box != null) {
                if ($(this.box).find('> div > div.wd-tree-div').length > 0) {
                    $(this.box)
                        .removeAttr('name')
                        .removeClass('wd-reset wd-tree')
                        .html('');
                }
                this.box = box;
            }
            ...
            $(this.box)
                .attr('name', this.name)
                .addClass('wd-reset wd-tree')
                .html('<div>'+
                        '<input id="tree_'+ this.name +'_focus" style="position: absolute; top: -10px; right: 0px; z-index: 1; '+
                        '       width: 1px; height: 1px; border: 0px; padding: 0px; opacity: 0"/>'+
                        '<div class="wd-tree-top"></div>' +
                        '<div class="wd-tree-div"></div>'+
                        '<div class="wd-tree-bottom"></div>'+
                    '</div>'
                );
            $(this.box).find('> div').css({
            ...
            if (this.topHTML !== '') {
                $(this.box).find('.wd-tree-top').html(this.topHTML);
                $(this.box).find('.wd-tree-div')
                    .css('top', $(this.box).find('.wd-tree-top').height() + 'px');
            }
            if (this.bottomHTML !== '') {
                $(this.box).find('.wd-tree-bottom').html(this.bottomHTML);
                $(this.box).find('.wd-tree-div')
                    .css('bottom', $(this.box).find('.wd-tree-bottom').height() + 'px');
            }
            // focus
            var kbd_timer;
            $(this.box).find('#tree_'+ this.name + '_focus')
                .on('focus', function (event) { 
        ...
        refresh: function (id) {
            ...
            if (node !== this) {
                ...
                $(this.box).find(tmp).before('<div id="tree_'+ this.name + '_tmp"></div>');
            ...
            function getNodeHTML(nd) {
                ...
                if (nd.group) {
                    html =
                        '<div class="wd-node-group" id="node_'+ nd.id +'"'+
                        '        onclick="wd[\''+ obj.name +'\'].toggle(\''+ nd.id +'\')"'+
                        ...
                        '</div>'+
                        '<div class="wd-node-sub" id="node_'+ nd.id +'_sub" style="'+ nd.style +';'+ (!nd.hidden && nd.expanded ? '' : 'display: none;') +'"></div>';
                    if (obj.flat) {
                        html = '<div class="wd-node-group" id="node_'+ nd.id +'"><span>&#160;</span></div>'+
                               '<div id="node_'+ nd.id +'_sub" style="'+ nd.style +';'+ (!nd.hidden && nd.expanded ? '' : 'display: none;') +'"></div>';
                    }
                } else {
                    if (nd.selected && !nd.disabled) obj.selected = nd.id;
                    tmp = '';
                    if (img) tmp  = '<div class="wd-node-image wd-icon '+ img +    (nd.selected && !nd.disabled ? " wd-icon-selected" : "") +'"></div>';
                    if (icon) tmp = '<div class="wd-node-image"><span class="'+ icon +'"></span></div>';
                    var text = nd.text;
                    if (typeof nd.text == 'function') text = nd.text.call(nd);
                    html =  '<div class="wd-node '+ (nd.selected ? 'wd-selected' : '') +' '+ (nd.disabled ? 'wd-disabled' : '') +'" id="node_'+ nd.id +'" style="'+ (nd.hidden ? 'display: none;' : '') +'"'+
                            '    ondblclick="wd[\''+ obj.name +'\'].dblClick(\''+ nd.id +'\', event);"'+
                            '    oncontextmenu="wd[\''+ obj.name +'\'].contextMenu(\''+ nd.id +'\', event);"'+
                            '    onClick="wd[\''+ obj.name +'\'].click(\''+ nd.id +'\', event); ">'+
                            '<table cellpadding="0" cellspacing="0" style="margin-left:'+ (level*18) +'px; padding-right:'+ (level*18) +'px"><tbody><tr>'+
                            '<td class="wd-node-dots" nowrap="nowrap" onclick="wd[\''+ obj.name +'\'].toggle(\''+ nd.id +'\'); '+
                            '        if (event.stopPropagation) event.stopPropagation(); else event.cancelBubble = true;">'+
                            '    <div class="wd-expand">'    + (nd.nodes.length > 0 ? (nd.expanded ? '-' : '+') : (nd.plus ? '+' : '')) + '</div>' +
                            '</td>'+
                            '<td class="wd-node-data" nowrap="nowrap">'+
                                    tmp +
                                    (nd.count || nd.count === 0 ? '<div class="wd-node-count">'+ nd.count +'</div>' : '') +
                                    '<div class="wd-node-caption">'+ text +'</div>'+
                            '</td>'+
                            '</tr></tbody></table>'+
                            '</div>'+
                            '<div class="wd-node-sub" id="node_'+ nd.id +'_sub" style="'+ nd.style +';'+ (!nd.hidden && nd.expanded ? '' : 'display: none;') +'"></div>';
                    if (obj.flat) {
                        html =  '<div class="wd-node '+ (nd.selected ? 'wd-selected' : '') +' '+ (nd.disabled ? 'wd-disabled' : '') +'" id="node_'+ nd.id +'" style="'+ (nd.hidden ? 'display: none;' : '') +'"'+
                                '    onmouseover="$(this).find(\'.wd-node-data\').w2tag(w2utils.base64decode(\''+ 
                                                w2utils.base64encode(text + (nd.count || nd.count === 0 ? ' - <span class="wd-node-count">'+ nd.count +'</span>' : '')) + '\'), '+
                                '               { id: \'' + nd.id + '\', left: -5 })"'+
                                '    onmouseout="$(this).find(\'.wd-node-data\').w2tag(null, { id: \'' + nd.id + '\' })"'+ 
                                '    ondblclick="wd[\''+ obj.name +'\'].dblClick(\''+ nd.id +'\', event);"'+
                                '    oncontextmenu="wd[\''+ obj.name +'\'].contextMenu(\''+ nd.id +'\', event);"'+
                                '    onClick="wd[\''+ obj.name +'\'].click(\''+ nd.id +'\', event); ">'+
                                '<div class="wd-node-data wd-node-flat">'+ tmp +'</div>'+
                                '</div>'+
                                '<div class="wd-node-sub" id="node_'+ nd.id +'_sub" style="'+ nd.style +';'+ (!nd.hidden && nd.expanded ? '' : 'display: none;') +'"></div>';
                    }
                }
    ...
    $.extend(w2tree.prototype, w2utils.event);
    w2obj.tree = w2tree;
})(jQuery);
    ...
    w2field.prototype = {
            ...
            if (['list'].indexOf(this.type) != -1) {
                ...
                setTimeout(function () {
                    ...
                    if (!$.isEmptyObject(selected) && options.icon) {
                        options.prefix = '<span class="wd-icon '+ options.icon +'"style="cursor: pointer; font-size: 14px;' +
            ...
            if (['enum', 'file'].indexOf(this.type) != -1) {
                ...
                for (var s in selected) {
                    ...
                    if (typeof options.renderItem == 'function') {
                        ren = options.renderItem(it, s, '<div class="wd-list-remove" title="'+ w2utils.lang('Remove') +'" index="'+ s +'">&#160;&#160;</div>');
                    } else {
                        ren = '<div class="wd-list-remove" title="'+ w2utils.lang('Remove') +'" index="'+ s +'">&#160;&#160;</div>'+
                ...
                if (html != '') {
                    ...
                    div.prepend('<div class="wd-enum-placeholder" style="'+ style +'">'+ $(obj.el).attr('placeholder') +'</div>');
                }
                // ITEMS events
                div.off('scroll.w2field').on('scroll.w2field', function (event) {
                    ...
                    .on('click', function (event) {
                        ...
                        if (eventData.isCancelled === true) return;
                        // default behavior
                        if ($(event.target).hasClass('wd-list-remove')) {
                        ...
                        if (obj.type == 'file' && !$(event.target).hasClass('wd-list-remove')) {
                            var preview = '';
                ...
                // adjust height
                $(this.el).height('auto');
                var cntHeight = $(div).find('> div.wd-multi-items').height() + w2utils.getSize(div, '+height') * 2;
            ...
            if (['list', 'combo', 'enum'].indexOf(obj.type) != -1) {
                ...
                switch (key) {
                    ...
                    case 39: // right
                        ...
                        break;
                    case 13: // enter
                        if ($('#wd-overlay').length == 0) break; // no action if overlay not open
                    ...
                    case 40: // down
                        ...
                        if (focus.val() == '' && $('#wd-overlay').length == 0) {
        ...
        keyUp: function (event) {
            ...
            if (['list', 'combo', 'enum'].indexOf(this.type) != -1) {
                ...
                if ([16, 17, 18, 20, 37, 39, 91].indexOf(event.keyCode) == -1) { // no refreah on crtl, shift, left/right arrows, etc
                    ...
                    if ($('#wd-overlay').length == 0 || [38, 40].indexOf(event.keyCode) == -1) { // no search on arrows
                        this.search();
                    }
                    // event after
                    this.trigger($.extend(edata, { phase: 'after' }));
        ...
        updateOverlay: function (indexOnly) {
            ...
            if (this.type == 'date') {
                if ($(obj.el).attr('readonly')) return;
                if ($('#wd-overlay').length == 0) {
                    $(obj.el).w2overlay('<div class="wd-reset wd-calendar" onclick="event.stopPropagation();"></div>', {
                ...
                (function refreshCalendar(month, year) {
                    $('#wd-overlay > div > div').html(obj.getMonthHTML(month, year, $(obj.el).val()));
                    $('#wd-overlay .wd-calendar-title')
                        .on('mousedown', function () {
                            if ($(this).next().hasClass('wd-calendar-jump')) {
                                $(this).next().remove();
                            } else {
                                var selYear, selMonth;
                                $(this).after('<div class="wd-calendar-jump" style=""></div>');
                                $(this).next().hide().html(obj.getYearHTML()).fadeIn(200);
                                setTimeout(function () {
                                    $('#wd-overlay .wd-calendar-jump')
                                        .find('.wd-jump-month, .wd-jump-year')
                                        .on('click', function () {
                                            if ($(this).hasClass('wd-jump-month')) {
                                                $(this).parent().find('.wd-jump-month').removeClass('selected');
                                                $(this).addClass('selected');
                                                selMonth = $(this).attr('name');
                                            }
                                            if ($(this).hasClass('wd-jump-year')) {
                                                $(this).parent().find('.wd-jump-year').removeClass('selected');
                                                $(this).addClass('selected');
                                                selYear = $(this).attr('name');
                                            }
                                            if (selYear != null && selMonth != null) {
                                                $('#wd-overlay .wd-calendar-jump').fadeOut(100);
                                                setTimeout(function () { refreshCalendar(parseInt(selMonth)+1, selYear); }, 100);
                                            }
                                        });
                                    $('#wd-overlay .wd-calendar-jump >:last-child').prop('scrollTop', 2000);
                                }, 1);
                            }
                        });
                    $('#wd-overlay .wd-date')
            ...
            if (this.type == 'time') {
                if ($(obj.el).attr('readonly')) return;
                if ($('#wd-overlay').length == 0) {
                    $(obj.el).w2overlay('<div class="wd-reset wd-calendar-time" onclick="event.stopPropagation();"></div>', {
                        css: { "background-color": "#fff" }
                    });
                }
                var h24 = (this.options.format == 'h24');
                $('#wd-overlay > div').html(obj.getHourHTML());
                $('#wd-overlay .wd-time')
                    .on('mousedown', function (event) {
                        $(this).css({ 'background-color': '#B6D5FB', 'border-color': '#aaa' });
                    ...
                    if (this.options.noMinutes == null || this.options.noMinutes === false) {
                        $('#wd-overlay .wd-time')
                            .on('mouseup', function () {
                                var hour = $(this).attr('hour');
                                if ($("#wd-overlay").length > 0) $('#wd-overlay')[0].hide();
                                $(obj.el).w2overlay('<div class="wd-reset wd-calendar-time"></div>', { css: { "background-color": "#fff" } });
                                $('#wd-overlay > div').html(obj.getMinHTML(hour));
                                $('#wd-overlay .wd-time')
                                    .on('mousedown', function () {
                                        $(this).css({ 'background-color': '#B6D5FB', 'border-color': '#aaa' });
                                    ...
                                    .on('mouseup', function () {
                                        setTimeout(function () { if ($("#wd-overlay").length > 0) $('#wd-overlay').removeData('keepOpen')[0].hide(); }, 10);
                                    });
                            });
                    } else {
                        $('#wd-overlay .wd-time')
            ...
            if (this.type == 'datetime') {
                ...
                if ($("#wd-overlay .wd-time").length > 0) $('#wd-overlay')[0].hide();
                if ($('#wd-overlay').length == 0) {
                    $(obj.el).w2overlay('<div class="wd-reset wd-calendar" onclick="event.stopPropagation();"></div>', {
                        css: { "background-color": "#f5f5f5" }
                    });
                ...
                var selDate = null;
                (function refreshCalendar(month, year) {
                    $('#wd-overlay > div > div').html(
                        obj.getMonthHTML(month, year, $(obj.el).val())
                        + (options.btn_now ? '<div class="wd-calendar-now now">'+ w2utils.lang('Current Date & Time') + '</div>' : '')
                    );
                    $('#wd-overlay .wd-calendar-title')
                        .on('mousedown', function () {
                            if ($(this).next().hasClass('wd-calendar-jump')) {
        ...
        */
                    ...
                    helper
                        ...
                        .on('click', function (event) {
                            if (obj.options.icon && typeof obj.onIconClick == 'function') {
                                // event before
                                var eventData = obj.trigger({ phase: 'before', type: 'iconClick', target: obj.el, el: $(this).find('span.wd-icon')[0] });
                                if (eventData.isCancelled === true) return;
        ...
        addSuffix: function () {
            ...
            setTimeout(function () {
                ...
                if (obj.options.arrows) {
                    ...
                    // add fresh
                    $(obj.el).after(
                        '<div class="wd-field-helper" style="border: 1px solid transparent">&#160;'+
                        '    <div class="wd-field-up" type="up">'+
                        '        <div class="arrow-up" type="up"></div>'+
                        '    </div>'+
                        '    <div class="wd-field-down" type="down">'+
        ...
        addFocus: function () {
            ...
            var html =
                '<div class="wd-field-helper">'+
                '    <div class="wd-icon icon-search" style="opacity: 0; display: none"></div>'+
            ...
            helper.find('input')
                .on('click', function (event) {
                    if ($('#wd-overlay').length == 0) obj.focus(event);
                    event.stopPropagation();
                })
                .on('focus', function (event) {
        ...
        addMulti: function () {
            ...
            var margin =
                ...
                'width          : ' + (w2utils.getSize(obj.el, 'width')
                                    ...
                                    + 'px;';
            if (obj.type == 'enum') {
                html =  '<div class="wd-field-helper wd-list" style="'+ margin + '; box-sizing: border-box">'+
                        '    <div style="padding: 0px; margin: 0px; display: inline-block" class="wd-multi-items">'+
                        '    <ul>'+
            ...
            if (obj.type == 'file') {
                html =  '<div class="wd-field-helper wd-list" style="'+ margin + '; box-sizing: border-box">'+
                        ...
                        '   </div>'+
                        '    <div style="position: absolute; padding: 0px; margin: 0px; display: inline-block" class="wd-multi-items">'+
            ...
            if (obj.type == 'file') {
                ...
                div.on('click', function (event) {
                    ...
                    .on('dragenter', function (event) {
                        if ($(obj.el).attr('readonly')) return;
                        $(div).addClass('wd-file-dragover');
                    })
                    .on('dragleave', function (event) {
                        if ($(obj.el).attr('readonly')) return;
                        var tmp = $(event.target).parents('.wd-field-helper');
                        if (tmp.length == 0) $(div).removeClass('wd-file-dragover');
                    })
                    .on('drop', function (event) {
                        if ($(obj.el).attr('readonly')) return;
                        $(div).removeClass('wd-file-dragover');
        ...
        getMonthHTML: function (month, year, selected) {
            ...
            var html  =
                '<div class="wd-calendar-title title">'+
                '    <div class="wd-calendar-previous previous"> <div></div> </div>'+
                '    <div class="wd-calendar-next next"> <div></div> </div> '+
                        months[month-1] +', '+ year +
                '</div>'+
                '<table class="wd-calendar-days" cellspacing="0"><tbody>'+
                '    <tr class="wd-day-title">' + dayTitle + '</tr>'+
                '    <tr>';
            ...
            for (var ci = 1; ci < 43; ci++) {
                if (weekDay === 0 && ci == 1) {
                    for (var ti = 0; ti < 6; ti++) html += '<td class="wd-day-empty">&#160;</td>';
                    ci += 6;
                } else {
                    if (ci < weekDay || day > daysCount[month-1]) {
                        html += '<td class="wd-day-empty">&#160;</td>';
                ...
                var DT  = new Date(dt);
                var className = '';
                if (DT.getDay() == 6) className  = ' wd-saturday';
                if (DT.getDay() == 0) className  = ' wd-sunday';
                if (dt == today) className += ' wd-today';
                ...
                html += '<td class="'+ (this.inRange(tmp_dt, true) ? 'wd-date ' + (tmp_dt_fmt == selected ? 'wd-date-selected' : '') : 'wd-blocked') + className + '" '+
        ...
        getYearHTML: function () {
            ...
            for (var m = 0; m < months.length; m++) {
                mhtml += '<div class="wd-jump-month" name="'+ m +'">'+ months[m] + '</div>';
            }
            for (var y = start_year; y <= end_year; y++) {
                yhtml += '<div class="wd-jump-year" name="'+ y +'">'+ y + '</div>';
            }
        ...
        getHourHTML: function () {
            ...
            for (var a = 0; a < 24; a++) {
                ...
                tmp[Math.floor(a/8)] += '<div class="'+ (this.inRange(tm1) || this.inRange(tm2) ? 'wd-time ' : 'wd-blocked') + '" hour="'+ a +'">'+ time +'</div>';
            }
            var html =
                '<div class="wd-calendar">'+
                '   <div class="wd-calendar-title">Select Hour</div>'+
                '   <div class="wd-calendar-time"><table><tbody><tr>'+
                '       <td>'+ tmp[0] +'</td>' +
                '       <td>'+ tmp[1] +'</td>' +
                '       <td>'+ tmp[2] +'</td>' +
                '   </tr></tbody></table></div>'+
        ...
        getMinHTML: function (hour) {
            ...
            for (var a = 0; a < 60; a += 5) {
                ...
                tmp[ind] += '<div class="'+ (this.inRange(tm) ? 'wd-time ' : 'wd-blocked') + '" min="'+ a +'">'+ time +'</div>';
            }
            var html =
                '<div class="wd-calendar">'+
                '   <div class="wd-calendar-title">Select Minute</div>'+
                '   <div class="wd-calendar-time"><table><tbody><tr>'+
                '       <td>'+ tmp[0] +'</td>' +
                '       <td>'+ tmp[1] +'</td>' +
                '       <td>'+ tmp[2] +'</td>' +
...
(function ($) {
    ...
    $.fn.w2form = function(method) {
        if ($.isPlainObject(method)) {
            ...
            if (object.formURL == '') {
                if (String(object.formHTML).indexOf('wd-page') == -1) {
                    object.formHTML = '<div class="wd-page page-0">'+ object.formHTML +'</div>';
                }
    ...
    w2form.prototype = {
            ...
            if (showErrors) {
                for (var e = 0; e < edata.errors.length; e++) {
                    ...
                    if (err.field.type == 'radio') { // for radio and checkboxes
                        $($(err.field.el).parents('div')[0]).w2tag(err.error, { "class": 'wd-error' });
                    } else if (['enum', 'file'].indexOf(err.field.type) != -1) {
                        (function (err) {
                            setTimeout(function () {
                                var fld = $(err.field.el).data('w2field').helpers.multi;
                                $(err.field.el).w2tag(err.error);
                                $(fld).addClass('wd-error');
                            }, 1);
                        })(err);
                    } else {
                        $(err.field.el).w2tag(err.error, { "class": 'wd-error' });
                    }
        ...
        generateHTML: function () {
            ...
            for (var f = 0; f < this.fields.length; f++) {
                ...
                // input control
                var input = '<input name="'+ field.name +'" class="wd-input" type="text" '+ field.html.attr +'/>';
                switch (field.type) {
                    case 'pass':
                    case 'password':
                        input = '<input name="' + field.name + '" class="wd-input" type = "password" ' + field.html.attr + '/>';
                        break;
                    case 'checkbox':
                        input = '<input name="'+ field.name +'" class="wd-input" type="checkbox" '+ field.html.attr +'/>';
                        break;
                    case 'radio':
                        input = '';
                        // normalized options
                        ...
                        // generate
                        for (var i = 0; i < items.length; i++) {
                            input += '<label><input name="' + field.name + '" class="wd-input" type = "radio" ' + field.html.attr + ' value="'+ items[i].id + '"/>' +
                                '&#160;' + items[i].text + '</label><br/>';
                        }
                        break;
                    case 'select':
                        input = '<select name="' + field.name + '" class="wd-input" ' + field.html.attr + '>';
                        // normalized options
                        var items =  field.options.items ? field.options.items : field.html.items;
                        if (!$.isArray(items)) items = [];
                        if (items.length > 0) {
                        ...
                        break;
                    case 'textarea':
                        input = '<textarea name="'+ field.name +'" class="wd-input" '+ field.html.attr +'></textarea>';
                        break;
                    case 'toggle':
                        input = '<input name="'+ field.name +'" type="checkbox" '+ field.html.attr +' class="wd-input wd-toggle"/><div><div></div></div>';
                        break;
                }
                if (field.html.group) {
                    if (group != '') html += '\n   </div>';
                    html += '\n   <div class="wd-group-title">'+ field.html.group + '</div>\n   <div class="wd-group">';
                ...
                html += '\n      <div class="wd-field '+ (field.html.span != null ? 'wd-span'+ field.html.span : '') +'" style="'+ field.html.style +'">'+
            ...
            for (var p = 0; p < pages.length; p++) pages[p] = '<div class="wd-page page-'+ p +'">' + pages[p] + '\n</div>';
            // buttons if any
            var buttons = '';
            if (!$.isEmptyObject(this.actions)) {
                ...
                for (var a in this.actions) { // it is an object
                    ...
                    if ($.isPlainObject(act)) {
                        ...
                        info.caption = a;
                        if (['save', 'update', 'create'].indexOf(a.toLowerCase()) != -1) info['class'] = 'wd-btn-blue'; else info['class'] = '';
                    }
                    buttons += '\n    <button name="'+ a +'" class="wd-btn '+ info['class'] +'" style="'+ info.style +'">'+
        ...
        resize: function () {
            ...
            if (edata.isCancelled === true) return;
            // default behaviour
            var main    = $(this.box).find('> div:not(.wd-lock-msg)');
            var header  = $(this.box).find('> div .wd-form-header');
            var toolbar = $(this.box).find('> div .wd-form-toolbar');
            var tabs    = $(this.box).find('> div .wd-form-tabs');
            var page    = $(this.box).find('> div .wd-page');
            var cpage   = $(this.box).find('> div .wd-page.page-'+ this.page);
            var dpage   = $(this.box).find('> div .wd-page.page-'+ this.page + ' > div');
            var buttons = $(this.box).find('> div .wd-buttons');
        ...
        refresh: function (field) {
            ...
            if (field != null) {
				...
				// default action
				$(this.box).find('.wd-page').hide();
				$(this.box).find('.wd-page.page-' + this.page).show();
				$(this.box).find('.wd-form-header').html(this.header);
            ...
            for (var f = from_field; f < to_field; f++) {
                ...
                // required
                if (field.required) {
                    $(field.el).parent().parent().addClass('wd-required');
                } else {
                    $(field.el).parent().parent().removeClass('wd-required');
                }
                // disabled
            ...
            for (var f = from_field; f < to_field; f++) {
                ...
                if (!$(field.el).hasClass('wd-input')) $(field.el).addClass('wd-input');
        ...
        render: function (box) {
            ...
            var html =  '<div>' +
                        (this.header != '' ? '<div class="wd-form-header">' + this.header + '</div>' : '') +
                        '    <div id="form_'+ this.name +'_toolbar" class="wd-form-toolbar" style="display: none"></div>' +
                        '    <div id="form_'+ this.name +'_tabs" class="wd-form-tabs" style="display: none"></div>' +
                            this.formHTML +
                        '</div>';
            $(this.box).attr('name', this.name)
                .addClass('wd-reset wd-form')
                .html(html);
            if ($(this.box).length > 0) $(this.box)[0].style.cssText += this.style;
...
;(function($){
     ...
     */
    $.fn.w2mask = function(label, delay){
        $(this).each(function() {
    ...
    $.maskElement = function(element, label){
        ...
        if(element.css("position") == "static") {
            element.addClass("wd-masked-relative");
        }
        
        element.addClass("wd-masked");
        
        var maskDiv = $('<div class="wd-loadmask"></div>');
        ...
        if(navigator.userAgent.toLowerCase().indexOf("msie 6") > -1){
            element.find("select").addClass("wd-masked-hidden");
        }
        
        element.append(maskDiv);
        
        if(label !== undefined) {
            var maskMsgDiv = $('<div class="wd-loadmask-msg" style="display:none;"></div>');
    ...
    $.unmaskElement = function(element){
        ...
        element.find(".wd-loadmask-msg,.wd-loadmask").remove();
        element.removeClass("wd-masked");
        element.removeClass("wd-masked-relative");
        element.find("select").removeClass("wd-masked-hidden");
    };
...
(function($) {
    ...
    $.MsgBox = {
        ...
        QUESTION    : 'wd-msgbox-icon-question',
        ERROR       : 'wd-msgbox-icon-error',
        INFO        : 'wd-msgbox-icon-info',
        WARNING     : 'wd-msgbox-icon-warning',
        ...
        fn_prompt : function(btn) {
            var text = $('#msgbox-input').val();
            this._calback(btn, text);
        },
        ...
        multiline : function(title, msg, callback) {
            var setting = {
                button: this.OKCANCEL,
                type: this.MULTILINE
            };
        ...
        show : function(options) {
            ...
            if (setting.fn !== null) {
                this._calback = setting.fn;
            } else {
                this._calback = function(key) {
                    $.MsgBox.hide();
                }
            }
            ...
            var fn = '$.MsgBox.fn';
            if (setting.type == this.PROMPT || setting.type == this.MULTILINE) {
                fn = '$.MsgBox.fn_prompt';
            }
            ...
            if (setting.buttons.length == 0) 
            {
                ...
                {
                    case this.OK :
                        button += '<button class="wd-btn" onclick="'+ fn +'(\'ok\')">OK</button>';                
                    break;

                    case this.YES :
                        button += '<button class="wd-btn" onclick="'+ fn +'(\'yes\')">Yes</button>';                
                    break;

                    case this.NO :
                        button += '<button class="wd-btn" onclick="'+ fn +'(\'no\')">No</button>';                
                    break;

                    case this.CANCEL :
                        button += '<button class="wd-btn" onclick="'+ fn +'(\'cancel\')">Cancel</button>';                
                    break;
            ...
            else
            {
                ...
                {
                    button += '<button class="wd-btn" onclick="'+ fn +'(\''+ setting.buttons[i] +'\')">'+ setting.buttons[i] +'</button>'; 
                }
            }

            if (setting.type == this.PROMPT) {
                setting.icon = ''; // set empty

                if (setting.height === 150) {
                    setting.height = 160;
                }

                setting.message += '<div style="margin-top: 5px"><input type="text" class="wd-input" style="width: 100%" id="msgbox-input" /></div>';
            } else if (setting.type == this.MULTILINE) {
                setting.icon = ''; // set empty
                ...
                setting.message += '<div style="margin-top: 5px"><textarea class="wd-input" style="width: 100%; height: 50px" id="msgbox-input"></textarea></div>';
            }

            if (setting.icon != '') {
                setting.message =  '<div class="wd-msgbox">'+ 
                    '<div class="wd-msgbox-icon '+ setting.icon +'"></div>'+
                    '<div class="wd-msgbox-body">'+ setting.message +'</div>'+
                '</div>';
            } else {
                setting.message = '<div class="wd-msgbox">'+ setting.message +'</div>';
            }
